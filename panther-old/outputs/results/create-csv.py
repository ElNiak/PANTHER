        ...
        if file.endswith(".iev"):
                        ...
                        with open(os.path.join(fol, err), "r") as f:
                            try:
                                ...
                                if "Using selector: EpollSelector" in content:
                                    ...
                                    "quinn" in content
                                    or "Running `target/debug/examples/server /panther-ivy/protocol-testing/quic/"
                                    in content
                                ):
                                    match = "quinn"
            ...
            with open(fullPath, "r") as f:
                ...
                if not "server" in file and os.path.isfile("res_client.txt"):
                    errors = lines_that_contain(
                        "require", os.path.join(fol, "res_client.txt")
                    )
                elif os.path.isfile("res_server.txt"):
                    errors = lines_that_contain(
                        "require", os.path.join(fol, "res_server.txt")
                    )
                if len(errors) > 0:
