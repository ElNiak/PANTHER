...
def get_template_path():
    return os.path.join(os.path.dirname(__file__), "../../panther/plugins/services/iut/quic/picoquic/templates/")
...
@pytest.mark.parametrize("cmd_args, env_vars, expected_snippet", [
    # Simple arguments
    (["/bin/sh", "-c", 'echo "Hello & Goodbye"'], {"PATH": "/usr/local/bin"}, 'echo \\"Hello & Goodbye\\"'),
    
    # Path with spaces and special characters
    (["python3", "-m", "panther", "--config", "/path/to/some & dir/file.txt"], {}, '--config /path/to/some\\ \\&\\ dir/file.txt'),
    
    # Environment variables with special characters
    (["echo", "test"], {"KEY": "value:with:colon", "PATH": "/usr/bin:/bin"}, 'KEY=value:with:colon'),
    
    # Command with quotes and ampersands
    (["openssl", "s_client", "-connect", "example.com:443", "-servername", "example.com"],
     {"SSL_CERT": "/path/to/cert.pem"}, 'openssl s_client -connect example.com:443 -servername example.com'),
...
@pytest.mark.parametrize("cmd_args, env_vars, extra_fields, expected_snippets", [
    (
...
def test_enhanced_command_template(enhanced_env, cmd_args, env_vars, extra_fields, expected_snippets):
    ...
    rendered = template.render(**params, command_args=cmd_args, env_vars=env_vars, extra_fields=extra_fields)
