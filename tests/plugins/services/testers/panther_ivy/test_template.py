...
@pytest.mark.parametrize("cmd_args,env_vars,expected_snippet,should_contain", [
    # Test regular arguments
    (
        ["/opt/panther_ivy/protocol-testing/apt/test_client", "server_addr=", "$TARGET_IP_HEX", ">", "/app/logs/testers.log", "2>", "/app/logs/testers.err"],
    ...
    (
        ["/opt/panther_ivy/test_client", "server_addr=", "192.168.1.1", "message='Hello & Goodbye'", ">", "/app/logs/log.txt"],
    ...
    (
        ["/opt/test client", "-c", "server_addr= $ADDR", "port= 8080", ">", "/path with spaces/log.txt"],
        {"ADDR": "192.168.1.1"},
        "\"/opt/test client\"",
        True
    ),
...
"""
    ...
    if should_contain:
        assert expected_snippet in rendered, f"Expected '{expected_snippet}' to be in rendered output"
    else:
        assert expected_snippet not in rendered, f"Expected '{expected_snippet}' NOT to be in rendered output"
    ...
    for arg in cmd_args:
        if ' ' in arg or '&' in arg or '"' in arg or "'" in arg:
            ...
            assert quoted_arg in rendered, f"Expected properly quoted arg '{quoted_arg}' in rendered output"
