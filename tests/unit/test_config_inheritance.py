...
class TestConfigInheritance:
    ...
    def test_network_environment_configs_inherit_correctly(self):
        """Test network environment configs inherit from NetworkEnvironmentPluginConfig."""
        # Docker Compose
        dc_config = DockerComposeConfig()
        assert isinstance(dc_config, NetworkEnvironmentPluginConfig)
        assert dc_config.type == "docker_compose"
        assert hasattr(dc_config, "enabled")  # From BasePluginConfig
        assert hasattr(dc_config, "network_name")  # From NetworkEnvironmentPluginConfig
        
        # Localhost Single Container (now fixed)
        lsc_config = LocalhostSingleContainerConfig()
        assert isinstance(lsc_config, NetworkEnvironmentPluginConfig)
        ...
        assert isinstance(sns_config, NetworkEnvironmentPluginConfig)
    ...
    def test_execution_environment_configs_inherit_correctly(self):
        """Test execution environment configs inherit from ExecutionEnvironmentPluginConfig."""
        strace_config = StraceConfig()
        assert isinstance(strace_config, ExecutionEnvironmentPluginConfig)
        assert hasattr(strace_config, "enabled")  # From BasePluginConfig
        assert hasattr(strace_config, "output_format")  # From ExecutionEnvironmentPluginConfig
