...
def docker_compose_env():
    ...
    with patch('panther.plugins.environments.network_environment.network_environment_interface.Environment') as mock_env:
...
def test_generate_entrypoint_with_structured_args(docker_compose_env):
    ...
    # Exercise
    with patch('panther.plugins.environments.network_environment.docker_compose.docker_compose.Path'):
...
def test_docker_compose_env_with_complex_commands(docker_compose_env):
    ...
    structured_cmds = kwargs['structured_commands']
    assert structured_cmds['run_cmd']['command_args'] == ['python3', '-c', "print('Hello & Goodbye')"]
