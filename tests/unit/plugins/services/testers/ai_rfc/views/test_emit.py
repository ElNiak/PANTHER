...
from panther.plugins.services.testers.ai_rfc.views.emit import ViewsError, emit_views
...
def test_verify_covers_member_patches(pipeline, tmp_path: Path):
    snapshot = _forge_pipeline(pipeline, tmp_path)
    from panther.plugins.services.testers.ai_rfc.views.emit import verify_views
