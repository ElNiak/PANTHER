...
def get_template_path():
    ...
    return os.path.join(plugin_dir, "panther", "plugins", "services", "testers", "panther_ivy", "templates")
...
@pytest.mark.parametrize("template_name,cmd_args,env_vars,expected_snippet", [
    ...
    (
        ...
        ["/opt/panther_ivy/test", "seed=123", "server_port=8080", "server_addr=$IVY_IP_HEX", ">", "/app/logs/test.log"],
    ...
    (
        ...
        ["/opt/panther_ivy/test_client", "seed=123", "server_addr=$TARGET_IP_HEX", "message='Hello & Goodbye'", ">", "/app/logs/test.log"],
    ...
    (
        ...
        ["/opt/panther_ivy/minip_test", "server_port=8080", "client_port=9090", "server_addr=10.0.0.1", ">", "/app/logs/test.log"],
        ...
        if "'" in str(cmd_args) or "&" in str(cmd_args) or "\"" in str(cmd_args):
            for arg in cmd_args:
                if any(c in str(arg) for c in "'&\""):
                    ...
                    assert literal_arg not in rendered or shlex.quote(literal_arg) in rendered, \
        ...
        for key, value in env_vars.items():
            if any(c in str(value) for c in "'&\":;"):
                ...
                if ":" in str(value) or ";" in str(value):
                    ...
                    assert literal_value not in rendered or shlex.quote(literal_value) in rendered, \
