...
def get_template_path():
    ...
    plugin_dir = Path(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.dirname(os.path.abspath(__file__)))))))
    return os.path.join(plugin_dir, "panther", "plugins", "services", "iut", "quic", "picoquic", "templates")
...
@pytest.mark.parametrize("template_name,cmd_args,env_vars,expected_snippet", [
    # Test server template with basic arguments
    (
        "server_command_structured.jinja",
        ["-c", "/certs/cert.pem", "-k", "/certs/key.pem", "-a", "h3", "-p", "4433", ">", "/app/logs/server.log"],
    ...
    (
        ...
        ["-c", "/path with spaces/cert.pem", "-k", "/path & special/key.pem", "localhost", "4433", ">", "/app/logs/client.log"],
    ...
    (
        "server_command_structured.jinja",
        ["-c", "/certs/cert.pem", "-k", "/certs/key.pem", "--message", "Hello \"World\" & Friends", "-p", "4433"],
...
def test_picoquic_structured_templates(env, template_name, cmd_args, env_vars, expected_snippet):
    ...
    try:
        ...
        assert expected_snippet in rendered, f"Expected '{expected_snippet}' not found in rendered template"
        ...
        for arg in cmd_args:
            if isinstance(arg, str) and any(c in arg for c in " '\"&|;<>()$`\\"):
                ...
                if " " in arg and "\"" not in arg:
                    assert f'"{arg}"' in rendered or f"'{arg}'" in rendered, \
                ...
                if any(c in arg for c in "&|;<>()$`\\"):
                    ...
                    for metachar in "&|;<>()$`\\":
                        if metachar in arg:
                            ...
                            assert metachar not in rendered or \
                                   f"\\{metachar}" in rendered or \
                                   safely_quoted in rendered or \
                                   arg not in rendered, \
        ...
        for key, value in env_vars.items():
            ...
            if isinstance(value, str) and any(c in value for c in "'\"`:;\\"):
                ...
                assert quoted_value in rendered or value not in rendered, \
...
@pytest.mark.parametrize("template_name,cmd_args,env_vars", [
    # Edge case: binary characters in arguments
    ("server_command_structured.jinja", ["-c", "/cert.pem", "-k", "/key.pem", b"\x00\x01\x02\x03"], {"TEST": "value"}),
    # Edge case: very long arguments
    ("client_command_structured.jinja", ["-c", "/c.pem", "-k", "/k.pem", "--very-long-option-" + "x" * 1000], {"LONG": "x" * 1000}),
    # Edge case: unicode characters
    ("server_command_structured.jinja", ["-c", "/c.pem", "-k", "/k.pem", "Hello 😀 World 🌎"], {"UNICODE": "😀🌎💫"})
])
