...
class TestObserverLogging:
    ...
    def test_setup_logging_uses_logger_factory(self):
        ...
        with patch('panther.core.observer.base.observer_interface.LoggerFactory.get_logger') as mock_get_logger:
    ...
    def test_setup_logging_respects_log_level(self):
        ...
        logger = observer._setup_logging(
            logger_name="TestObserver",
            log_level=logging.DEBUG  # Request DEBUG level
        )
    ...
    def test_setup_logging_with_file_output(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('panther.core.observer.base.observer_interface.LoggerFactory.add_file_handler') as mock_add_file:
                logger = observer._setup_logging(
                ...
                mock_add_file.assert_called_once_with(Path(str(log_file)), level=None)
    ...
    def test_setup_logging_without_file_output(self):
        ...
        with patch('panther.core.observer.base.observer_interface.LoggerFactory.add_file_handler') as mock_add_file:
            logger = observer._setup_logging(
                logger_name="NoFileObserver",
                log_level=logging.INFO,
                output_file=None
            )
            
            # Should not call add_file_handler
    ...
    def test_multiple_observers_consistent_format(self):
        ...
        # Both should propagate to root (consistent formatting)
    ...
    def test_real_observer_implementations(self):
        ...
        try:
            ...
            assert logger_obs.logger.name == "LoggerObserver"
    ...
    def test_concurrent_observer_creation(self):
        ...
        def create_observer(idx):
            observer = TestObserver()
            logger = observer._setup_logging(
                logger_name=f"ConcurrentObserver{idx}",
                log_level=logging.INFO
            )
            with lock:
    ...
    def test_observer_logging_level_override(self):
        ...
        observer = TestObserver()
        logger = observer._setup_logging(
            logger_name="OverrideObserver",
            log_level=logging.DEBUG  # Override to DEBUG
        )
        
        # Logger should be at DEBUG level
        assert logger.level == logging.DEBUG
        
        # But root logger should still be at INFO
        root_logger = logging.getLogger()
