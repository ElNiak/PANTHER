...
class TestLoggerFactory:
    ...
    def test_get_logger_auto_initialize(self):
        ...
        # Auto-init uses ERROR as default level for early loggers
        assert LoggerFactory._config["level"] == "ERROR"
    ...
    def test_get_logger_creates_consistent_format(self):
        ...
        # All should have same effective level (set to DEBUG for file capture)
        ...
        # LoggerFactory creates loggers with propagate=False and their own handlers
        assert not logger1.propagate
        assert not logger2.propagate
        assert not logger3.propagate

        # Each logger should have at least one handler (console)
    ...
    def test_update_level_dynamically(self):
        ...
        # Root logger is always set to DEBUG to capture all messages;
        # individual console handlers filter at the configured level
        root_logger = logging.getLogger()
        assert root_logger.level == logging.DEBUG

        LoggerFactory.update_level("DEBUG")
        # update_level changes the config and the root logger level
    ...
    def test_color_formatter_with_colorlog(self):
        ...
        # colorlog is imported inside _create_formatter via contextlib.suppress
        # so we need to patch it in sys.modules
        with patch.dict(sys.modules, {"colorlog": mock_colorlog}):
            config = {"level": "INFO", "format": "%(message)s", "enable_colors": True}
            LoggerFactory.initialize(config)
    ...
    def test_fallback_formatter_without_colorlog(self):
        ...
        # Remove colorlog from sys.modules to simulate it not being installed
        ...
        try:
            ...
            formatter = LoggerFactory._create_formatter()
            assert isinstance(formatter, logging.Formatter)
        finally:
            if saved is not None:
                sys.modules["colorlog"] = saved
            elif "colorlog" in sys.modules:
    ...
    def test_file_handler_addition(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # add_file_handler adds to the root logger.
            # LoggerFactory.get_logger creates loggers with propagate=False,
            # so we use the root logger directly to verify the file handler works.
            root_logger = logging.getLogger()
