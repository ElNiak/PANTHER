...
class MockServiceManager:
    def __init__(self):
        ...
        self.service_config_to_test = type('obj', (object,), {
        ...
        self.run_cmd = {
            "setup_cmds": [
                "TARGET_IP=$(getent hosts " + self.service_targets + r' | tail -n 1 | awk "{ print \$1 }");',
                'echo "Resolved ' + self.service_targets + ' IP - $TARGET_IP" >> /app/logs/ivy_setup.log;',
                ...
                'echo "Resolved ' + self.service_targets + ' IP in hex - $TARGET_IP_HEX" >> /app/logs/ivy_setup.log;',
                'echo "Resolved ' + self.service_name + ' IP in hex - $IVY_IP_HEX" >> /app/logs/ivy_setup.log;',
                " ",
                "rm -rf /opt/panther_ivy/protocol-testing/apt/build/*;" if True else f"rm -rf /opt/panther_ivy/protocol-testing/{self.protocol.name}/build/*;",
            ]
        }
        self.environments = {}
        self.volumes = []
