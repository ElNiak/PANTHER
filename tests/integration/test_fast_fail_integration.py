...
from panther.config.config_global_schema import GlobalConfig, FastFailConfig
...
class TestExperimentManagerFastFail:
    ...
    def test_experiment_initialization_error_triggers_fast_fail(
        self, mock_test_case, global_config, mock_event_manager, mock_observer_factory, tmp_path
    ):
        ...
        with patch('panther.core.experiment_manager.EmitterRegistry'), \
            ...
            mock_test_case.side_effect = ExperimentInitializationError(
                "Failed to initialize test case",
                experiment_name="test_experiment"
            )
...
class TestFastFailEndToEnd:
    ...
    def mock_environment(self):
        ...
        with patch('docker.from_env') as docker_mock, \
    ...
    def test_docker_build_failure_stops_experiment(self, mock_environment, tmp_path):
        ...
        assert isinstance(manager.fast_fail_handler.critical_error, DockerBuildException)
