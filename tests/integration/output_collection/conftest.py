...
def output_validator():
    ...
    class OutputValidator:
        ...
        def validate_enhanced_patterns(log_dir: Path):
            ...
            additional_ssl = [f for f in log_dir.glob("*") if "ssl" in f.name.lower() or "key" in f.name.lower()]
            additional_pcap = [f for f in log_dir.glob("*") if any(ext in f.name for ext in [".cap", "capture", "packet"])]
            
            return len(additional_ssl) > 1 or len(additional_pcap) > 1  # More than just standard files
...
def experiment_runner():
    ...
    class ExperimentRunner:
        ...
        def run_experiment(self, config_file: Path, output_dir: Path, timeout: int = 300):
