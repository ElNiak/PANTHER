...
class TestOutputCollectionEdgeCases:
    ...
    def test_docker_compose_path_resolution(self, temp_output_dir, mock_environment_config, mock_event_manager):
    ...
    @pytest.mark.unit
    @pytest.mark.output_collection
    def test_localhost_path_resolution_fallback(self, temp_output_dir, mock_environment_config, mock_event_manager):
    ...
    @pytest.mark.unit
    @pytest.mark.output_collection
    def test_shadow_ns_path_resolution_priority(self, temp_output_dir, mock_environment_config, mock_event_manager):
    ...
    def test_enhanced_pattern_discovery(self, temp_output_dir):
        ...
        with patch.object(BaseNetworkEnvironment, '_discover_additional_outputs') as mock_discover:
            
            def mock_discover_impl(log_dir):
                ssl_patterns = ["*ssl*key*", "*keylog*", "*tls*key*", "*key*log*", "*.keys", "*sslkey*"]
            ...
            assert len(discovered_files) >= len(ssl_pattern_files) + len(pcap_pattern_files) + 3  # +3 for standard files
    ...
    def test_missing_files_graceful_handling(self, temp_output_dir):
        ...
        with patch.object(BaseNetworkEnvironment, '_register_service_outputs') as mock_register:
            ...
            def mock_register_impl(service_name):
                ...
                try:
                    ...
                    if not log_dir.exists():
                        mock_env.logger.warning(f"Log directory not found for service {service_name}")
                        return
                    ...
                    for filename in ["stdout.log", "stderr.log", "sslkeylogfile.txt"]:
                        ...
                        if file_path.exists():
                            mock_env.output_collector.register_output_file(str(file_path), service_name)
                        else:
                            mock_env.logger.warning(f"Expected file {filename} not found for {service_name}")
                    
                except Exception as e:
                    mock_env.logger.error(f"Error registering outputs for {service_name}: {e}")
    ...
    def test_empty_files_handling(self, temp_output_dir):
        ...
        # Create empty files
        empty_files = ["stdout.log", "stderr.log", "sslkeylogfile.txt", "test_service.pcap"]
        ...
        try:
            ...
            try:
                with open(restricted_file, 'r') as f:
                    content = f.read()
                mock_collector.register_output_file(str(restricted_file), "test_service")
...
class TestOutputCollectionPerformance:
    ...
    def test_output_collection_timing(self, temp_output_dir):
        ...
        assert collection_time < 1.0, f"Output collection took {collection_time:.3f}s (too slow)"
    ...
    def test_large_file_handling(self, temp_output_dir):
        ...
        assert processing_time < 0.5, f"Large file processing took {processing_time:.3f}s (too slow)"
    ...
    def test_concurrent_service_collection(self, temp_output_dir):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_services) as executor:
            futures = [executor.submit(create_service_outputs, i) for i in range(num_services)]
        ...
        assert concurrent_time < 2.0, f"Concurrent collection took {concurrent_time:.3f}s (too slow)"
