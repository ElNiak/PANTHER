...
# Ivy related
IVY_INCLUDE_PATH = os.path.join(SOURCE_DIR,"/pfv-ivy/ivy/include/1.7/")
MODEL_DIR        = os.path.join(SOURCE_DIR,"/pfv-ivy/protocol-testing/")
...
def restore_config():
    with open('config/config.ini', 'w') as configfile:
        with open('config/default_config.ini', "r") as default_config:
    ...
    if experiment_parameters:
        ...
        for arg in experiment_parameters:
            if arg in config['global_parameters']:
                logging.info(f"Updating global parameter: {arg} : {experiment_parameters[arg]}")
                config.set('global_parameters', arg, experiment_parameters[arg])
            if arg in config['debug_parameters']:
                logging.info(f"Updating debug parameter: {arg} : {experiment_parameters[arg]}")
                config.set('debug_parameters', arg, experiment_parameters[arg])
            if arg == "net_parameters":
                net_args = experiment_parameters[arg]
                logging.info(f"Updating network parameter: {net_args} : {experiment_parameters[arg]}")
...
def update_protocol_config(protocol_argument, current_protocol, current_tests):
    ...
    for arg in protocol_argument:
        if arg in protocol_conf[current_protocol+'_parameters']:
            ...
            protocol_conf.set(current_protocol+'_parameters', arg, protocol_argument[arg])
...
def get_experiment_config(new_current_protocol=None, get_all_test=False, get_default_conf=False):
    config = configparser.ConfigParser(allow_no_value=True)
    config.read('configs/config.ini')
    ...
    tests_enabled,conf_implementation_enable, implementation_enable, protocol_model_path, protocol_results_path, protocol_test_path, protocol_conf = get_protocol_config(config, current_protocol,get_all_test,get_default_conf,is_apt)
    
    return current_protocol, tests_enabled, conf_implementation_enable, implementation_enable, protocol_model_path, protocol_results_path, protocol_test_path, config, protocol_conf


def get_protocol_config(config, protocol, get_all_test=False, get_default_conf=False,is_apt=False):
    protocol_conf = configparser.ConfigParser(allow_no_value=True)
    ...
    if  is_apt:
        ...
        config.set('global_parameters', "tests_dir", os.path.join(MODEL_DIR, "apt", "apt_tests"))
        config.set('global_parameters', "dir"      , os.path.join(MODEL_DIR, "apt", "test", 'temps'))
        config.set('global_parameters', "build_dir", os.path.join(MODEL_DIR, "apt", "build"))
        
        protocol_test_path    = os.path.join(protocol_model_path, "apt","tests/")
        protocol_results_path = os.path.join(protocol_model_path, "apt","test/","temp/")
    else:
        protocol_model_path   = os.path.join(MODEL_DIR, protocol)
        config.set('global_parameters', "tests_dir", os.path.join(MODEL_DIR, protocol, protocol + "_tests"))
        config.set('global_parameters', "dir"      , os.path.join(MODEL_DIR, protocol, "test", 'temps'))
        config.set('global_parameters', "build_dir", os.path.join(MODEL_DIR, protocol, "build"))
    ...
    for file_path in os.listdir(implem_config_path_server):
        ...
        if os.path.isfile(os.path.join(implem_config_path_server, file_path)):
            ...
            conf_implementation_enable[implem_name] = [implem_conf_server, implem_conf_client]
    ...
    return tests_enabled,conf_implementation_enable, implementation_enable, \
        protocol_model_path, protocol_results_path, protocol_test_path, protocol_conf
