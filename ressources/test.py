...
servers = [
    ...
    ['quiche',[scdir+'/quiche/','cargo run --manifest-path=tools/apps/Cargo.toml --bin quiche-server --  --cert tools/apps/src/bin/cert.crt --early-data --dump-packets '+os.environ.get('QUIC_IMPL_DIR',os.environ.get('PROOTPATH',''))+'/qlogs/dump_packet.txt --key tools/apps/src/bin/cert.key --no-retry --listen 127.0.0.1:4443' ]], #--early-data 
            ...
            for test_command in range(0,iters):
                ...
                if "quic_server_test_retry" in test.name: 
                    if quic_name == "quant":
                        ...
                        quic_cmd = './picoquicdemo -r -l - -D -L -q '+os.environ.get('QUIC_IMPL_DIR',os.environ.get('PROOTPATH',''))+'qlog_picoquic' 
                if "quic_server_test_version_negociation" in test.name: 
                    if quic_name == "quant":
                        ...
                        quic_cmd = './picoquicdemo -l - -D -L -q '+os.environ.get('QUIC_IMPL_DIR',os.environ.get('PROOTPATH',''))+'qlog_picoquic' 
