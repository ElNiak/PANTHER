...
class ServiceManagerEventMixin:
    ...
    def _get_service_context(self) -> Tuple[str, str]:
        """Build (service_id, service_name) from instance attributes.

        Returns:
            Tuple of (service_id, service_name)
        """
    ...
    def notify_service_event(
        ...
        # Events emitted directly (no validation wrapper exists)
        if event_name == "service_destroyed":
            ...
            self.event_emitter.emit_service_preparation_completed(
                service_id=service_id,
                service_name=service_name,
                duration_seconds=(details.get("duration_seconds") if details else None),
        ...
        # Compilation events → tester analysis events
        ...
        # Build events → Docker build events
            ...
            logger.warning(
                "Unknown service event '%s' for service '%s'. "
                "Consider adding proper event mapping.",
                event_name,
                service_name,
            )
        ...
        if step_id.startswith("deploy"):
            if success:
                self.notify_service_deployment_completed(
                    environment=(
                        result.get("environment", "default") if result else "default"
                    ),
            ...
            if success:
                self.notify_service_ready(readiness_checks={step_id: True})
            else:
                self.event_emitter.emit_service_error(
                    service_id=service_id,
                    service_name=service_name,
