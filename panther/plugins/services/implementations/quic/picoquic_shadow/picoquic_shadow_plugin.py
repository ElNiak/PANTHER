...
class PicoquicShadowPlugin(IProtocolPlugin):
    ...
    def get_service_manager(self, implementation_name: str):
        if implementation_name not in self.service_managers:
            ...
            if implementation_name == "picoquic_shadow":
                from .service_manager import PicoquicShadowServiceManager
                self.service_managers[implementation_name] = PicoquicShadowServiceManager()
            else:
