...
class PicoquicShadowServiceManager(
    ...
    def __init__(
        ...
        self.logger.debug(
            "Initializing Picoquic service manager for '%s'", implementation_name
        )
        self.logger.debug(
            "Loaded Picoquic configuration: %s", self.service_config_to_test
        )
    ...
    def _do_prepare(self, plugin_manager: "PluginManager | None" = None):
        ...
        try:
            ...
            self.build_docker_image_with_manager(
                plugin_manager, "panther_base", "service"
            )
