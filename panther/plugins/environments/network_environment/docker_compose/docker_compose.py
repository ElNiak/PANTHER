...
class DockerComposeEnvironment(INetworkEnvironment):
        ...
        try:
            ...
            for service in self.services_managers:
                ...
                if "ivy" in service.service_name:
                    self.logger.debug(
                        "Adding wait for Ivy testers to be ready for %s",
            ...
            for service in self.services_managers:
                self.logger.debug(
                    "Generating entrypoint script for service %s",
                    service.service_name
                )
    ...
    def wait_tester_command(self, other_service):
        ...
        other_service.run_cmd["post_compile_cmds"] = other_service.run_cmd[
                                "post_compile_cmds"
                            ] + [
    ...
    def _combine_shell_constructs(self, command_list):
        ...
        self.logger.debug("Processing %d commands in shell construct combination", len(filtered_commands))
        ...
        construct_patterns = {
            "while": {
                ...
                "patterns": [
                    # while condition; do
                    lambda s: s.strip().lower().startswith("while "),
                ]
            },
            "for": {
                "start": ["for"], 
                ...
                "patterns": [
                    # for var in list; do
                    lambda s: s.strip().lower().startswith("for "),
                ]
            },
            "if": {
                "start": ["if"], 
                ...
                "patterns": [
                    # if condition; then
                    lambda s: s.strip().lower().startswith("if "),
                ]
            },
            "case": {
                "start": ["case"], 
                ...
                "patterns": [
                    # case var in
                    lambda s: s.strip().lower().startswith("case "),
                ]
            },
            "function": {
                "start": ["function", "() {", "(){"], 
                ...
                "patterns": [
                    ...
                    # name() { ... }
                    lambda s: ("() {" in s or "(){" in s) and not s.strip().lower().startswith("function "),
                    # function name { ... }
        ...
        for i, cmd in enumerate(filtered_commands):
            ...
            for construct, patterns in construct_patterns.items():
                ...
                if "patterns" in patterns:
                    for pattern_func in patterns["patterns"]:
                        if pattern_func(cmd):
                            ...
                            self.logger.debug("Starting %s construct (pattern match): %s", 
                                             construct, cmd.strip()[:40])
                            break
                    if starts_construct:
                        break
                ...
                if not starts_construct and any(p in cmd_lower for p in patterns["start"]):
                    ...
                    if construct == "function":
                        ...
                        # 1. function name() { ... }
                        # 2. name() { ... }
                        if "function " in cmd_lower or \
                           (any(cmd_lower.find(f"{c}") > -1 for c in ["() {", "(){", "()\n{", "() \n{"])) or \
                           (cmd_lower.rstrip().endswith("{") and "(" in cmd_lower and ")" in cmd_lower):
                            starts_construct = True
                            active_constructs.append(construct)
                            self.logger.debug("Starting %s construct (keyword match): %s", 
                                             construct, cmd.strip()[:40])
                            break
                    # For other constructs, check if it's the first word
                        ...
                        starts_construct = True
                        active_constructs.append(construct)
                        self.logger.debug("Starting %s construct (first word match): %s", 
                                         construct, cmd.strip()[:40])
                        break
                ...
                if any(p in cmd_lower for p in end_patterns):
                    ...
                    if current_construct == "function" and "}" in cmd_lower:
                        ...
                        if cmd_lower == "}" or cmd_lower.endswith("}") or cmd_lower.endswith("};"):
                            ...
                            self.logger.debug("Ending %s construct: %s", 
                                             current_construct, cmd.strip()[:40])
            ...
            if (ends_construct and not active_constructs) or (not starts_construct and not active_constructs):
                ...
                if len(construct_buffer) == 1 and not starts_construct and not ends_construct:
                    ...
                    if has_shell_command:
                        ...
                        if shell_cmd_template:
                            ...
                            if "\n" in multiline_cmd and ("() {" in multiline_cmd.split("\n")[0] or 
                                                       "(){" in multiline_cmd.split("\n")[0] or
                                                       "function " in multiline_cmd.lower().split("\n")[0]):
                                ...
                                self.logger.debug(f"Setting safer function description for combined construct: {fn_name}")
                    ...
                    self.logger.debug("Combined %s into multiline command: %s", 
                                     construct_type, 
                                     multiline_cmd.split("\n")[0].strip()[:40] + "...")
                construct_buffer = []
        ...
        if construct_buffer:
            ...
            self.logger.debug("Combined remaining commands into multiline command: %s", 
                             multiline_cmd.split("\n")[0].strip()[:40] + "...")
        
        # Final validation - make sure we don't return empty result
        if not result:
            self.logger.warning("Shell construct combination resulted in empty list. Original commands: %s",
            ...
            if any(obj is not None for obj in original_objects):
                return [obj if obj is not None else cmd 
        ...
        for cmd_type, cmds in service.run_cmd.items():
            ...
            if cmd_type == "run_cmd":
                ...
                if run_cmd.get("command_binary"):
                    self.logger.debug(
                        "Using command binary: %s", run_cmd["command_binary"]
                    )
                ...
                if isinstance(run_cmd.get("command_args"), list):
                    ...
                    self.logger.debug(
                        "Using command args list: %s", run_cmd["command_args"]
                    )
                    ...
                    try:
                        ...
                        self.logger.debug(
                            "Using command args from string: %s", cmd_parts
                        )
                        # Ensure all parts are strings
                        command_args.extend(cmd_parts)
                    except (AttributeError, TypeError) as e:
                        self.logger.warning("Error converting command args: %s. Using empty list.", e)
                ...
                if run_cmd.get("environment"):
                    try:
                        ...
                        self.logger.debug(
                            "Using environment variables: %s", env_vars
                        )
                    except (AttributeError, TypeError) as e:
                ...
                processed_commands["run_cmd"] = {
                    ...
                    "command_binary": run_cmd.get("command_binary", ""),  # Keep command_binary separate
                ...
                if isinstance(cmds, list):
                    ...
                    self.logger.debug(
                        ...
                        len(valid_cmds), len(combined_cmds), cmd_type
                    )
                    ...
                    if len(valid_cmds) > 0 and len(combined_cmds) > 0:
                        ...
                        if isinstance(valid_cmds[0], str):
                            before_sample = valid_cmds[0].strip()[:40] + ("..." if len(valid_cmds[0]) > 40 else "")
                        else:
                            # For ShellCommand objects, use the command property
                            cmd_text = valid_cmds[0].command
                            before_sample = cmd_text.strip()[:40] + ("..." if len(cmd_text) > 40 else "")
                    ...
                    if not combined_cmds:
                        self.logger.warning("Combined commands resulted in empty list for %s, using original commands", cmd_type)
                        ...
                        if not combined_cmds:  # Double check we have something
                            self.logger.error("No valid commands available for %s after combination attempt", cmd_type)
                            processed_commands[cmd_type] = []
                            continue
                    ...
                    for cmd in combined_cmds:
                        if isinstance(cmd, ShellCommand):
                            ...
                            if not cmd.command or cmd.command.strip() == "":
                                self.logger.warning(
                                    "ShellCommand object has no command: %s", cmd
                                )
                                continue
                            self.logger.debug(
                                "Processing ShellCommand: %s", cmd.to_dict()
                            )
                            ...
                            if "\n" in cmd:
                                ...
                                if (("() {" in cmd or "(){" in cmd or "function " in cmd.lower()) and "}" in cmd):
                                    shell_cmd.is_function_definition = True
                                    # Extract function name for better description
                                    fn_name = first_line.split('(')[0].replace('function ', '').strip()
                                    # Use plain function name without special characters in description
                                    shell_cmd.description = f"Function: {fn_name}"
                                    self.logger.debug("Detected function definition in combined construct: %s", 
                                                    first_line + "...")
                                ...
                                # Check for explicit function definition syntax variations
                                    ...
                                    shell_cmd.description = f"Function: {fn_name}"
                                    self.logger.debug("Detected function definition (function keyword): %s", 
                                                    first_line + "...")
                                    
                                # Function definition with name() { syntax across multiple lines
                                elif "(" in first_line and ")" in first_line and (
                                    ...
                                    # Use plain function name without special characters in description
                                    shell_cmd.description = f"Function: {fn_name}"
                                    self.logger.debug("Detected function definition (name() syntax): %s", 
                                                    first_line + "...")
                                    
                                # Check for control structures
                                elif (first_line.startswith("if ") or 
                                      first_line.startswith("for ") or 
                                      first_line.startswith("while ") or 
                                      first_line.startswith("case ")):
                                    shell_cmd.is_control_structure = True
                                    self.logger.debug("Detected control structure in combined construct: %s",
                                                    first_line + "...")
                            ...
                            self.logger.debug(
                                "Processing string command: %s", shell_cmd.to_dict()
                            )
                            ...
                            if not shell_cmd.command or shell_cmd.command.strip() == "":
                                ...
                                continue
                            self.logger.debug(
                                "Processing dict command: %s", shell_cmd.to_dict()
                            )
                            processed_list.append(shell_cmd.to_dict())
                        else:
                            ...
                            try:
                                ...
                                self.logger.warning("Could not convert command to ShellCommand: %s, error: %s", cmd, e)
                                # Skip this command
                    ...
                    if not shell_cmd.command or shell_cmd.command.strip() == "":
                        ...
                        continue
                    self.logger.debug(
                        "Processing single string command: %s", shell_cmd.to_dict()
                    )
                    processed_commands[cmd_type] = [shell_cmd.to_dict()]
                else:
                    # Empty or unsupported type
                    self.logger.debug(
                        "Processing empty or unsupported command type: %s", cmd_type
                    )
                    processed_commands[cmd_type] = []
        ...
        with open(
            ...
            with open(
                ...
                try:
                    ...
                    self.logger.error("Failed to tear down Docker Compose environment: %s", e.stderr)
                    raise e

    def _preprocess_service_commands(self, service):
        """
        Preprocess service command objects to ensure compatibility with templates.

        This method converts ShellCommand objects to their string representation
        with appropriate markers for special command types (non-critical, function definitions).
        
        NOTE: This method is now DEPRECATED and should not be called directly.
        Command preprocessing is handled centrally by the generate_from_template method.

        Args:
            service: The service manager instance with commands to preprocess

        Returns:
            The original service object - no preprocessing performed
        """
        # DEPRECATED - No preprocessing here to avoid duplicate command conversions
        # Command processing is now handled entirely by generate_from_template
        self.logger.warning("_preprocess_service_commands is deprecated and should not be used.")
        return service