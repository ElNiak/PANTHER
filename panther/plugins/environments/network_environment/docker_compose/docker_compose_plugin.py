...
class DockerComposeEnvironment(INetworkEnvironment):
    def __init__(self, config_path:str, network_driver: str = "bridge",
        ...
        self.services_network_config_file_path = os.path.join(os.getcwd(), "plugins", "environments","network_environment" ,"docker_compose", "docker-compose.generated.yml")
        self.network_name = "quic_network_dynamic"
        self.network_driver = network_driver
        self.templates_dir = templates_dir
        self.compose_file_path = Path("docker-compose.generated.yml")
        self.services = {}
    ...
    def find_free_port(self, start_port: int = 5000, end_port: int = 6000, assigned_ports:set = None) -> int:
        """
        Finds a free port within the specified range.
        """
        for port in range(start_port, end_port):
            if self.is_port_free(port) and (assigned_ports is None or port not in assigned_ports):
                return port
    ...
    def configure_network(self, services: Dict[str, Dict[str, Any]]):
        ...
        for service_name, config in services.items():
            ...
            service_type = service_name.split('_')[-1].upper()  # Assuming naming convention like 'picoquic_server_rfc9000_panther'
            ...
            for port_mapping in ports:
                try:
                    ...
                    self.logger.error(f"Invalid port mapping '{port_mapping}' for service '{service_name}'. Skipping.")
                    continue

                if host_port in assigned_ports or not self.is_port_free(host_port):
                    self.logger.warning(f"Host port {host_port} is in use. Finding an alternative port.")
                    try:
                        new_host_port = self.find_free_port(assigned_ports=assigned_ports)
                        self.logger.info(f"Assigning new host port {new_host_port} for container port {container_port}.")
                        adjusted_ports.append(f"{new_host_port}:{container_port}")
                        assigned_ports.add(new_host_port)
                    except RuntimeError as e:
                        self.logger.error(f"Failed to assign a free port for {service_name}: {e}")
                        raise e
                else:
            ...
            service_def = {
                ...
                'ports': adjusted_ports if self.network_driver != "host" else [],  # Ports are not needed in host mode
        ...
        if len(all_host_ports) != len(set(all_host_ports)):
            self.logger.error("Duplicate host ports detected in the Docker Compose configuration.")
            raise ValueError("Duplicate host ports detected in the Docker Compose configuration.")

        with open(self.services_network_config_file_path, 'w') as compose_file:
            self.logger.debug(f"Writing Docker Compose file to '{self.services_network_config_file_path}' with contents: {compose_dict}")
            yaml.safe_dump(compose_dict, compose_file, default_flow_style=False)
        
        self.logger.info(f"Docker Compose file generated at '{self.services_network_config_file_path}'")
    ...
    def setup_environment(self, services: Dict[str, Dict[str, Any]], deployment_commands: Dict[str, str]):
        """
    ...
    def generate_docker_compose(self):
        ...
        rendered = template.render(services=self.services, deployment_commands=self.deployment_commands)
        with open(self.compose_file_path, 'w') as f:
            f.write(rendered)
        self.logger.info(f"Docker Compose file generated at '{self.compose_file_path}'")
    ...
    def launch_docker_compose(self):
        ...
        try:
            ...
            self.logger.error(f"Failed to launch Docker Compose environment: {e.stderr.decode()}")
            raise e
    ...
    def teardown_environment(self):
        ...
        try:
            ...
            self.logger.error(f"Failed to tear down Docker Compose environment: {e.stderr.decode()}")
            raise e
    ...
    def read_compose_file(self) -> Dict[str, Any]:
        ...
        if not os.path.exists(self.services_network_config_file_path):
            self.logger.error(f"Docker Compose file '{self.services_network_config_file_path}' does not exist.")
            raise FileNotFoundError(f"Docker Compose file '{self.services_network_config_file_path}' does not exist.")

        with open(self.services_network_config_file_path, 'r') as compose_file:
            return yaml.safe_load(compose_file)
        
    def __str__(self) -> str:
        return super().__str__() + f" (network_driver={self.network_driver}, network_name={self.network_name})"