...
log "Executing variable assignment: TARGET_IP=$(getent hosts picoquic_server | awk '{print $1}' | grep -v '^127' | head -n 1)"
# Use eval to properly execute these special command types while preserving their syntax
eval "TARGET_IP=$(getent hosts picoquic_server | awk '{print $1}' | grep -v '^127' | head -n 1)" || {
  exit $?
}
...
FIRST_OUTPUT="/app/logs/ivy_client_$cmd_type-cmd9_first_part.log"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Output from $cmd_type command #9 first part:" >> /app/logs/ivy_client_commands.log
...
if [ $FIRST_STATUS -eq 0 ]; then
  ...
  REST_OUTPUT="/app/logs/ivy_client_$cmd_type-cmd9_rest.log"
  ...
  # Log the captured output
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Output from $cmd_type command #9 rest part:" >> /app/logs/ivy_client_commands.log
...
# Handle special command types: variable assignment, shell builtin, control structure, or nested quotes
log "Executing shell builtin: cd /opt/panther_ivy/protocol-testing/quic/quic_tests/server_tests;"
# Use eval to properly execute these special command types while preserving their syntax
eval "cd /opt/panther_ivy/protocol-testing/quic/quic_tests/server_tests;" || {
  exit $?
}
...
# Handle command with && operator
log "Command contains && operator: cp /opt/panther_ivy/protocol-testing/quic/build/quic_server_test_stream /app/logs/quic_server_test_stream && "

# Extract parts of the command before and after &&
FIRST_PART="cp /opt/panther_ivy/protocol-testing/quic/build/quic_server_test_stream /app/logs/quic_server_test_stream "
REST_OF_CMD=" "

# Create output files for this command
FIRST_OUTPUT="/app/logs/ivy_client_$cmd_type-cmd1_first_part.log"
...
# Log the captured output
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Output from $cmd_type command #1 first part:" >> /app/logs/ivy_client_commands.log
cat "$FIRST_OUTPUT" >> /app/logs/ivy_client_commands.log

if [ $FIRST_STATUS -eq 0 ]; then
  # If first part succeeded, execute rest of command
  log "First part succeeded. Executing rest: ${REST_OF_CMD}"
  REST_OUTPUT="/app/logs/ivy_client_$cmd_type-cmd1_rest.log"
  ...
  # Log the captured output
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Output from $cmd_type command #1 rest part:" >> /app/logs/ivy_client_commands.log
  cat "$REST_OUTPUT" >> /app/logs/ivy_client_commands.log
