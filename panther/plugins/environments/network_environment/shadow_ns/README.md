...
The Shadow NS environment follows a multi-stage Docker build process similar to other network environments:

1. **Base Image Building**: Builds from `panther/plugins/services/Dockerfile` with environment-specific tagging
2. **Service Image Verification**: Ensures all required service images are available before deployment
3. **Shadow Container Creation**: Creates the Shadow simulator container with proper configuration
...
- Verifies service images before deployment
...
- Inspect generated Dockerfile and shadow.yml configuration files in the output directory
- Verify Docker images are properly built using `docker images | grep shadow`
