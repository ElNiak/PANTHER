...
class GeneralConfig(BaseModel):
    ...
    model_unblocked_syscall_latency: bool = Field(
        default=False,
        description="Add latency for unblocked system calls"
    )
...
class ExperimentalConfig(BaseModel):
    ...
    strace_logging_mode: str = Field(
        default="standard",
        description="Options: 'none', 'standard', 'detailed'"
    )
...
class NetworkNodeConfig(BaseModel):
    ...
    bandwidth_down: str = Field(
        default="100 Gbit",
        description="Download bandwidth"
    )
    bandwidth_up: str = Field(
        default="100 Gbit",
...
class NetworkEdgeConfig(BaseModel):
    ...
    latency: int = Field(
        default=10,
        description="Latency of the edge in milliseconds"
    )
    jitter: int = Field(
        default=10,
        description="Jitter of the edge in milliseconds"
    )
...
class NetworkGraphConfig(BaseModel):
    ...
    type: str = Field(
        default="gml",
        description="Options: '1_gbit_switch', 'gml'"
    )
    ...
    edges: List[NetworkEdgeConfig] = Field(
        default_factory=list,
        description="List of network edges"
    )
...
class NetworkConfig(BaseModel):
    ...
    latency: int = Field(
        default=10,
        description="Latency of the network in milliseconds"
    )
    jitter: int = Field(
        default=10,
        description="Jitter of the network in milliseconds"
    )
    packet_loss: float = Field(
        default=0.0,
        description="Packet loss rate"
...
class HostOptionDefaultsConfig(BaseModel):
    ...
    pcap_enabled: bool = Field(
        default=True,
        description="Enable PCAP capture for all hosts"
    )
...
class ShadowNSConfig(NetworkEnvironmentPluginConfig):
    ...
    host_option_defaults: HostOptionDefaultsConfig = Field(
        ...
        description="Default host options"
    )
    hosts: HostsConfig = Field(
        default_factory=HostsConfig,
