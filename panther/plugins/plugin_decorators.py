...
def register_plugin(
    ...
    **kwargs,
    ...
    def decorator(cls: type) -> type:
        ...
        # Create manifest with enhanced metadata
        ...
        logger.debug(
            "Registered plugin: %s v%s (type: %s)",
            plugin_name,
            version,
            plugin_type.value,
        )
...
def get_plugin_by_name(name: str, plugin_type: Optional[str] = None) -> Optional[Tuple[type, PluginManifest]]:
    """
...
def get_plugins_by_type(plugin_type: str) -> Dict[str, Tuple[type, PluginManifest]]:
    ...
    for plugin_id, (cls, manifest) in _DECORATED_PLUGINS.items():
        if manifest.type.value == plugin_type:
            plugins[manifest.name] = (cls, manifest)
    return plugins
...
def validate_decorated_plugins() -> Dict[str, List[str]]:
    ...
    for plugin_id, (cls, manifest) in _DECORATED_PLUGINS.items():
        ...
        if manifest.version and not manifest.version.replace(".", "").replace("-", "").isalnum():
...
def get_version_configs(plugin_name: str) -> Dict[str, Any]:
    """
    Get all available version configurations for a plugin.
    ...
    """
    return _VERSION_CONFIGS.get(plugin_name, {}).copy()
...
def get_protocol_by_name(name: str, protocol_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
