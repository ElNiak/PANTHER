...
def is_development_mode() -> bool:
    ...
    if hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
        site_packages.append(Path(sys.prefix) / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages")
...
def get_plugin_directory(plugin_type: str, in_development_mode: Optional[bool] = None) -> Path:
    """
...
def get_template_directory(plugin_type: str) -> Path:
    ...
    if in_development_mode:
        ...
        try:
            ...
            import importlib.resources
            template_dir_obj = importlib.resources.files(f"panther.plugins.{plugin_dir}.tutorials").joinpath("template")
            ...
            if spec.origin is None:
                raise ImportError(f"Could not find origin for module path: panther.plugins.{plugin_dir}.tutorials")
...
def create_subplugin(plugin_type: str, plugin_name: str, subplugin_type: str, 
                     in_development_mode: Optional[bool] = None) -> bool:
    """
    ...
    try:
        ...
        for item in subplugin_template_dir.glob('*'):
            ...
            if item.is_dir() and item.name not in [subplugin_type, subplugin_type.replace('_', '')]:
                continue
...
def create_plugin(plugin_type: str, plugin_name: str, in_development_mode: Optional[bool] = None,
                 create_subplugins: bool = False) -> bool:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--create", nargs=2, metavar=("TYPE", "NAME"),
                        help="Create a new plugin. TYPE can be service, environment, or protocol. NAME is the plugin name.")
    parser.add_argument("--tutorial", choices=["service", "environment", "protocol"],
