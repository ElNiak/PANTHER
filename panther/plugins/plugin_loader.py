    ...
    def build_docker_image(self, impl_name: str, versions: str):
        ...
        if impl_name in self.dockerfiles:
            ...
            image_tag = self.docker_builder.build_image(
                ...
                config={} if not hasattr(versions, "version") else {
            ...
            if image_tag:
                ...
                self.logger.error(f"Image build failed for implementation '{impl_name}' version '{versions}'")
        else:
            self.logger.error(f"Dockerfile not found for implementation '{impl_name}' in {self.dockerfiles}. Skipping.")
            exit(1)
        
    def build_docker_image_from_path(self, path: Path, name: str,
