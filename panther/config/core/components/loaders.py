        ...
        if '_include' in config:
            ...
            for include in includes:
                ...
                config = OmegaConf.merge(
                    OmegaConf.create(include_config),
...
class UnifiedVersionLoader(BaseLoader):
    ...
    def load(self, source: Any) -> Dict[str, Any]:
        ...
        raise NotImplementedError("Use discover_versions or load_version_config instead")
    ...
    def load_version_config(
        ...
        version_config = self._load_version_file(impl_name, impl_type, protocol, version)
    ...
    def _load_version_file(
        ...
        # Try different paths
        paths = [
            self.plugin_dir / "services" / impl_type / protocol / impl_name / "versions" / f"{version}.yaml",
            self.plugin_dir / "services" / impl_type / protocol / impl_name / "config" / f"version_{version}.yaml",
            self.plugin_dir / "protocols" / "client_server" / protocol / "versions" / f"{version}.yaml",
            self.plugin_dir / "protocols" / "peer_to_peer" / protocol / "versions" / f"{version}.yaml",
        ]
...
class PluginConfigLoader(BaseLoader):
    ...
    def _find_plugin_config(self, plugin_name: str) -> Optional[Path]:
        ...
        locations = [
            ...
            self.plugin_dir / "services" / "testers" / "**" / plugin_name / "config.yaml",
    ...
    def _find_plugin_schema(self, plugin_name: str) -> Optional[Path]:
        ...
        for pattern in patterns:
            matches = list(self.plugin_dir.glob(str(pattern).replace(str(self.plugin_dir) + "/", "")))
            if matches:
                return matches[0]
