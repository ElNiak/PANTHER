...
class SchemaValidator(BaseValidator):
    ...
    def validate(self, config: Any) -> ValidationResult:
        ...
        try:
            import jsonschema
            jsonschema.validate(config if isinstance(config, dict) else config.to_dict(), schema)
    ...
    def validate_against_schema(self, data: Dict[str, Any], schema: Dict[str, Any]) -> ValidationResult:
    ...
    def _get_schema(self, config: Any) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            JSON schema or None
        """
        config_type = config.__class__.__name__ if hasattr(config, '__class__') else type(config).__name__
...
class BusinessRulesValidator(BaseValidator):
    ...
    def _validate_service_rules(self, service: Any) -> ValidationResult:
        ...
        if hasattr(service, 'protocol') and service.protocol.name.lower() in ["quic", "https"]:
            if not getattr(service, 'generate_new_certificates', False):
                result.add_warning("certificates", "TLS protocols should generate certificates")
                            ...
                            for port_mapping in service.ports:
                                ...
                                if host_port in all_ports:
                                    result.add_warning(
                                        ...
                                        f"Port {host_port} already used by {all_ports[host_port]}"
                                    )
                                else:
                                    all_ports[host_port] = f"{test.name}/{service_name}"
    ...
    def _validate_global_rules(self, global_config: Any) -> ValidationResult:
        ...
        # Validate log levels
        if hasattr(global_config, 'logging') and hasattr(global_config.logging, 'level'):
            valid_levels = ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]
            if global_config.logging.level.upper() not in valid_levels:
                result.add_error("logging.level", f"Invalid log level: {global_config.logging.level}")
        ...
        if hasattr(global_config, 'paths'):
            ...
            if hasattr(paths, 'plugin_dir'):
                ...
                if not Path(paths.plugin_dir).exists():
                    result.add_warning("paths.plugin_dir", f"Plugin directory not found: {paths.plugin_dir}")
