...
``panther.config.core.manager.ConfigurationManager`` is::
    ...
    class ConfigurationManager(
        ...
        CachingMixin,                # 4. In-memory caching & cache stats
        ErrorHandlerMixin,           # 5. Error handling (from panther.core)
    ):
        ...
...
MRO (Method Resolution Order) notes
    ...
    * ``CachingMixin`` defines ``__init__`` that initialises cache state.
