...
class ValidationOperationsMixin(LoggerMixin):
    ...
    def validate_experiment_config(
        self, config: Union[Dict, "ExperimentConfig"]
    ) -> ValidationResult:
        ...
        # Schema validation (Pydantic handles this automatically during instantiation)
        # No need to call validate() again since the config was already validated during creation
    ...
    def validate_global_config(
        self, config: Union[Dict, "GlobalConfig"]
    ) -> ValidationResult:
    ...
    def _run_validators(
        self, config: Any, validators: List[Validator]
    ) -> ValidationResult:
        ...
        for validator in validators:
            try:
                ...
                result.add_error(
                    "validator", f"{validator.__class__.__name__} failed: {e}"
                )

        return result

    def _validate_experiment_specific(
    ...
    def _validate_global_specific(self, config: "GlobalConfig") -> ValidationResult:
        ...
        if hasattr(config, "paths"):
            if not config.paths.output_dir:
                result.add_error(
                    "paths.output_dir", "Output directory must be specified"
                )
