...
class StateManagementMixin(LoggerMixin):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "plugins": {
                "discovered": (
                    len(self.discover_plugins())
                    if hasattr(self, "discover_plugins")
                    else 0
                ),
                "schemas_loaded": (
                    len(getattr(self, "_schema_cache", {}))
    ...
    def get_validation_report(self) -> Dict[str, Any]:
        ...
        report = {
            "validation_mode": (
                "strict" if getattr(self, "_strict_mode", False) else "lenient"
            ),
        ...
        if hasattr(self, "validation_results"):
            for config_name, result in self.validation_results.items():
                ...
                for error in result.errors:
                    error_type = (
                        error.field.split(".")[0] if "." in error.field else error.field
                    )
                    report["common_errors"][error_type] = (
                        report["common_errors"].get(error_type, 0) + 1
                    )
    ...
    def export_configuration(self, format: str = "yaml") -> str:
        ...
        if (
            hasattr(self, "current_experiment_config")
            and self.current_experiment_config
        ):
        ...
        if hasattr(self, "discover_plugins"):
            ...
            for plugin_name, metadata in plugins.items():
                try:
                    ...
                    test_config = {
                        "implementation": {"name": plugin_name, "type": metadata.type}
                    }
    ...
    def check_configuration_health(self) -> HealthReport:
        ...
        if hasattr(self, "validation_results"):
            failed_validations = sum(
                1 for result in self.validation_results.values() if not result.is_valid
            )
            if failed_validations > 0:
                report.add_warning(
                    f"{failed_validations} configurations failed validation"
                )
            else:
        ...
        if hasattr(self, "discover_plugins"):
            try:
                ...
                for plugin in plugins.values():
                    ...
                    summary["by_type"][plugin_type] = (
                        summary["by_type"].get(plugin_type, 0) + 1
                    )
