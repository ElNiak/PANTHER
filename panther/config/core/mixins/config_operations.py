...
class ConfigOperationsMixin(LoggerMixin):
    ...
    def merge_configurations(
        ...
        *configs: Any,
        ...
        self.logger.debug(f"Merging {len(configs)} configurations with strategy {strategy.value}")
    ...
    def apply_configuration_patch(self, base: BaseConfig, patch: Dict[str, Any]) -> BaseConfig:
        ...
        if conflict == ConflictResolution.ERROR:
            ...
            for key in override:
                if key in base:
                    ...
                    if isinstance(base_val, DictConfig) and isinstance(override_val, DictConfig):
                        # Recursive check
                        self._merge_with_conflict_resolution(base_val, override_val, conflict)
            ...
            for key, value in override.items():
                if key in result:
                    ...
                    elif isinstance(base_val, DictConfig) and isinstance(value, DictConfig):
                        # Recursive merge
                        result[key] = self._merge_with_conflict_resolution(base_val, value, conflict)
                    else:
                        # Replace
                        result[key] = value
                else:
