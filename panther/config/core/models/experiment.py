...
class StepsConfig(BaseUnifiedModel):
    ...
    pre_commands: List[str] = Field(default_factory=list, description="Commands to run before test")
    wait: int = Field(60, description="Wait time in seconds")
    post_commands: List[str] = Field(default_factory=list, description="Commands to run after test")
...
class TestConfig(BaseUnifiedModel):
    ...
    network_environment: NetworkEnvironmentConfig = Field(..., description="Network environment configuration")
    execution_environment: List[ExecutionEnvironmentConfig] = Field(
        default_factory=list,
        description="Execution environment configurations"
    )
    ...
    fast_fail_enabled: Optional[bool] = Field(None, description="Override fast-fail for this test")
    continue_on_failure: bool = Field(False, description="Continue test on service failures")
...
class ExperimentConfig(BaseUnifiedModel):
    ...
    metadata: Optional[ExperimentMetadata] = Field(None, description="Experiment metadata")
