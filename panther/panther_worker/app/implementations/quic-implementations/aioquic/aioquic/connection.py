...
class QuicConnection:
    ...
    def _handle_path_response_frame(
        ...
        self._logger.debug("Chris: %s - %s", data, context.network_challenge.local_challenge)
        self._logger.debug("Chris: %s", context)
	...
	#chris  After changing the address to which it sends non-probing packets, an
   	#       endpoint can abandon any path validation for other addresses

        self._network_challenges = [QuicNetworkPathChallenge()]
        #context.network_challenge.is_validated = True
