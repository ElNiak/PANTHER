...
class IvyTest(object):
    ...
    def prep_gperf(self, iteration):
        if self.config["debug_parameters"].getboolean("gperf"):
            f = open(
                os.path.join(self.dir, self.name + str(iteration)) + "_cpu.prof", "w"
            )
            f.write("")
            f.close()
            f = open(
                os.path.join(self.dir, self.name + str(iteration)) + "_heap.prof", "w"
            )
            f.write("")
            f.close()
    ...
    def run_gperf(self, command):
        if self.config["debug_parameters"].getboolean("gperf"):
            ...
            os.system(
                ...
                + " "
                + os.path.join(
                    self.config["global_parameters"]["dir"], self.name + str(i)
                )
                + "_cpu.prof > "
                + os.path.join(
                    self.config["global_parameters"]["dir"], self.name + str(i)
        ...
        if platform.system() != "Windows":
            ...
            if self.config["debug_parameters"].getboolean("gperf"):
                os.system(
                    ...
                    + self.name
                    + " "
                    + os.path.join(self.dir, self.name + str(iteration))
                    + "_cpu.prof > "
                    + os.path.join(self.dir, self.name + str(iteration))
                    + "_cpu.pdf"
                )
    ...
    def generate_tester_command(self, iteration, iclient):
        ...
        gperf_cmd = (
            "LD_PRELOAD=/usr/local/lib/libprofiler.so CPUPROFILE="
            + os.path.join(self.dir, self.name + str(iteration))
            + "_cpu.prof "
            if self.config["debug_parameters"].getboolean("gperf")
            else ""
