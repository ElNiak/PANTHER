            ...
            for line in input_file_content_lines:
                ...
                if "implementation of" in line:
                    ...
                    if "quic_transport_parameters" in splitted_line[0]:
                        ...
                        has_implem, is_init, is_module_object, is_module_object_present = init_mapping(
                        ...
                        if not is_init:
                            ...
                            if not "returns" in signature:
                                ...
                                if "(" in signature:
                                    ...
                                    get_action_parameters(action_parameters, current_elem)
                                else:
                                    if not is_implement:
                        ...
                        if has_implem:
                            ...
                            called_actions = get_action_implementation(content, implem_elem, line, None, splitted_line)
                                    ...
                                    for file in mapping.keys():
                                        if file != "quic_transport_parameters.ivy":
                                            if "." in called_action:
                                                ...
                                                current_action, found = find_external_object_action(called_action,
                                                                                                                current_elem, file,
                                                                                                                found, mapping)
                                            else:
                                                current_action, found = find_external_action(called_action,
                                                                                                        current_elem,
                                                                                                        file, found, mapping)
                    ...
                    if another_file:
                        ...
                        for file in mapping.keys():
                            ...
                            if file != "quic_transport_parameters.ivy" and not found and file != splitted_line[0]:
                                if "." in action_name:
                                    # TODO uniformize frame and module and object
                                    current_action, found = find_external_object_action(action_name,
                                                                                                current_action, file,
                                                                                                found, mapping)
                                else:
                                    current_action, found = find_external_action(action_name, current_action,
                                                                                        file, found, mapping)
                            if found:
                                break
                    if DEBUG:
                    ...
                    if current_action and signature_type in ["around", "after", "before"]:
                        ...
                        called_actions = get_action_implementation(content, implem_elem, line, new_file, splitted_line)
                        if len(called_actions) > 0:
                            for called_action in called_actions:
                                for file in mapping.keys():
                                    if file != "quic_transport_parameters.ivy":
                                    ...
                                    if  object_name + "_name" in obj.keys():
                                        for act in obj["actions"]:
                                            if act["action_name"] == action_name:
                                                if DEBUG:
                                                    logger.info("Found 2")
                                                in_action = setup_assertions(act, in_action,
                                ...
                                for obj in mapping[file]:
                                    if not isinstance(obj["actions"], list): # else mean it is a module
                                        if obj["actions"]["action_name"] == action_name:
                                            if DEBUG: 
                                                logger.info("Found 3")
                                            in_action = setup_assertions(obj["actions"], in_action,
                                                                                in_action_assumptions,
                                                                                in_action_guarantees, line, mapping)
                                            found = True
                                            break
                                                    ...
                                                    for file_i in mapping.keys():
                                                        if file_i != "quic_transport_parameters.ivy":
                                                            if "." in action_name:
                                                                ...
                                                                current_action, found = find_external_action(action_name,
                                                                                                                        current_elem,
...
def init_mapping(action_name, content, has_implem, is_init, is_module_object,
    ...
    if "." in action_name:
        ...
        is_module_object_present = check_object_present(action_name, content,
                                                        is_module_object_present, line,
                                                        mapping, object_name,
                                                        splitted_line)

        if not is_module_object_present:
            get_module_object_attributes(action_name, content, line,
