...
class IvyServer:
    ...
    def __init__(self, dir_path=None, experiments=None):
        ...
        IvyServer.ivy_model_path = dir_path + "/panther-ivy/protocol-testing/quic"
        IvyServer.ivy_test_path = (
            dir_path + "/panther-ivy/protocol-testing/quic/quic_tests/"
        )
        IvyServer.ivy_temps_path = (
            dir_path + "/panther-ivy/protocol-testing/quic/test/temp/"
        )
        IvyServer.ivy_include_path = dir_path + "/panther-ivy/ivy/include/1.7/"
        IvyServer.local_path = (
            os.environ["ROOT_PATH"] + "/panther-ivy/protocol-testing/quic/test/temp/"
        )
    ...
    def progress():
        if IvyServer.implems_used is not None:
            ...
            res = requests.post(
                "http://" + IvyServer.current_implem + "-ivy:80/progress"
            )
            ...
            if res.text != "None":
                if int(res.text) == IvyServer.experiments.args.iter:
                    if (
                        ...
                        < len(TESTS_CUSTOM) * IvyServer.experiments.args.iter
                    ):  # TODO some bugs
                        IvyServer.current_count += int(res.text)
                    else:
                        if len(IvyServer.implems_used) > 0:
                            IvyServer.current_implem = IvyServer.implems_used.pop()
                            IvyServer.experiments.args.implementations = [
                                IvyServer.current_implem
                            ]
    ...
    def serve_index():
        ...
        if request.method == "POST":
            ...
            IvyServer.experiments.args.initial_version = (
                1
                if request.form["version"] == "rfc9000"
                else (29 if request.form["version"] == "draft29" else 28)
            )
            IvyServer.experiments.args.alpn = request.form["ALPN"]
            IvyServer.experiments.args.vnet = (
                True if request.form["net_mode"] == "vnet" else False
            )
            ...
            IvyServer.app.logger.info(IvyServer.experiments.args)
            data = IvyServer.experiments.args.__dict__
            IvyServer.experiments.args.ivy_ui = (
                True if "ivy_ui" in request.form.keys() else False
            )
            data["tests"] = TESTS_CUSTOM
            res = requests.post(
                "http://" + IvyServer.current_implem + "-ivy:80/run-exp", json=data
            )
            IvyServer.app.logger.info(res.text)
            ...
            return render_template(
                ...
                implems=IvyServer.implems,
                progress=0,
                iteration=int(IvyServer.experiments.args.iter)
                * (len(IvyServer.implems_used) + 1)
                * len(TESTS_CUSTOM),
            )  # TODO 0rtt
        else:
            if IvyServer.implems_used is not None:
    ...
    def serve_results():
        ...
        IvyServer.current_exp = IvyServer.ivy_temps_path + str(
            IvyServer.nb_exp - int(page)
        )
            ...
            if "ivy_stderr.txt" in file:
                ...
                try:
                    ...
                    generate_graph_input(
                        IvyServer.current_exp + "/" + file, plantuml_file
                    )
        ...
        return render_template(
            ...
            pcap_frame_link="http://ivy-visualizer:80/?file=http://ivy-standalone:80/directory/"
            + str(IvyServer.nb_exp - int(page))
            + "/file/"
            + pcap_file
            ...
            qlog_frame_link="http://ivy-visualizer:80/?file=http://ivy-standalone:80/directory/"
            + str(IvyServer.nb_exp - int(page))
            + "/file/"
            + qlog_file
            if qlog_file != ""
    ...
    def serve_results_global():
        ...
        summary = {}
        df_csv = pd.read_csv(
            IvyServer.ivy_temps_path + "data.csv", parse_dates=["date"]
        )
                ...
                for line in input_file_content_lines:
                    ...
                    if "implementation of" in line:
                        ...
                        if "quic_transport_parameters" in splitted_line[0]:
                            ...
                            line = int(
                                splitted_line[1].replace("line", "").replace(" ", "")
                            )
                            action_name = (
                                splitted_line[2].replace(" ", "").replace("\n", "")
                            )
                            print(action_name)
                            ...
                            if "." in action_name:
                                ...
                                if "frame." in action_name:
                                    ...
                                    mapping[splitted_line[0]].append(
                                        {
                                            "frame_name": action_name.replace(
                                                "frame.", ""
                                            ).split(".")[0],
                                            "frame_object": [],
                                            "actions": {
                                ...
                                "import " in content[line - 1]
                                and not "_finalize" in action_name
                            ):
                                has_implem = False
                                mapping[splitted_line[0]].append(
                                    {
                            ...
                            with open(prefix + splitted_line[0], "r") as f:
                                ...
                                while (
                                    not (
                                        action_name.split(".")[-1]
                                        in corrected_signature
                                        and (
                                            "action" in corrected_signature
                                            or "implement" in corrected_signature
                                        )
                                    )
                                    and corrected_line < len(content)
                                    and corrected_line >= 0
                                ):
                                    # ("action" not in corrected_signature or "implement" not in corrected_signature) and \
                                    # print('////')
                                    corrected_line += 1
                                    corrected_signature = content[corrected_line - 1]
                                    # print(corrected_line)
                                    # print(action_name.split(".")[-1] not in corrected_signature)
                                    # print("action" not in corrected_signature or "implement" not in corrected_signature)
                                    # print(corrected_line < len(content))
                                    # print(corrected_line >= 0)
                                    # print(not (action_name.split(".")[-1]  in corrected_signature and ("action" in corrected_signature or "implement"  in corrected_signature)) and corrected_line < len(content) and corrected_line >= 0)
                                    # print("---------------------")
                                    if action_name.split(".")[
                                        -1
                                    ] in corrected_signature and (
                                        "action" in corrected_signature
                                        or "implement" in corrected_signature
                                    ):
                                        line = corrected_line
                                ...
                                if is_module_object:
                                    if not is_module_object_present:
                                        ...
                                        current_elem = mapping[splitted_line[0]][-1][
                                            "actions"
                                        ][-1]
                                    else:
                                ...
                                if not is_init:
                                    ...
                                    if not "returns" in signature:
                                        ...
                                        action_return = (
                                            signature.split("returns")[1]
                                            .split("(")[1]
                                            .split(")")[0]
                                            .split(":")
                                        )
                                        current_elem["action_return"][
                                            "name"
                                        ] = action_return[0]
                                        current_elem["action_return"][
                                            "type"
                                ...
                                if has_implem:
                                    ...
                                    for l in content[line:]:
                                        ...
                                        if "{" in l:
                                            bracket_count += 1
                                        current_elem["implementation"].append(l)
                            print(splitted_line[0])
                            print(mapping[splitted_line[0]])
                            print("============================================")
    ...
    def get_quic_model():
        ...
        """
        base_path_test = os.path.join(
            IvyServer.ivy_model_path, "quic_tests/client_tests/quic_client_test.ivy"
        )
        model = {}
        ...
        while len(includes_to_add) > 0:
            ...
            for dir in os.listdir(IvyServer.ivy_model_path):
                ...
                if (
                    os.path.isdir(os.path.join(IvyServer.ivy_model_path, dir))
                    and ("quic_" in dir or "tls_" in dir)
                    and dir != "quic_tests"
                ):  #  and dir != 'quic_fsm'
                    if dir not in model:
                        model[dir] = {}
                    for file in os.listdir(IvyServer.ivy_model_path + "/" + dir):
                        ...
                        if ".ivy" in file and file in includes_to_add:
                            ...
                            with open(
                                IvyServer.ivy_model_path + "/" + dir + "/" + file, "r"
                            ) as f:
                                content = f.read()
                                for line in content.splitlines():
                                    line = line.lstrip()
                                    if line.startswith("include "):
                                        ...
                                        include_ivy = (
                                            include_ivy.split(" ")[-1] + ".ivy"
                                        )
                                        if include_ivy != ".ivy":
                                            if (
                                                include_ivy not in includes_to_done
                                                and include_ivy not in includes_to_add
                                            ):
                                                includes_to_add.append(include_ivy)
                                            model[dir][file]["include"].append(
                                                include_ivy
                                            )
                                    elif line.startswith("function "):
                                        IvyServer.app.logger.info(line)
                                        include_ivy = line.split("#")[0]
                                        include_ivy = include_ivy.replace(
                                            "function ", ""
                                        )
                                        if include_ivy != ".ivy":
                                            model[dir][file]["function"].append(
                                                include_ivy
                                            )
                                        ...
                                        if include_ivy != ".ivy":
                                            model[dir][file]["relation"].append(
                                                include_ivy
                                            )
    ...
    def get_json_graph():
        ...
        with open("/tmp/cytoscape_config.json", "r") as json_file:
            ...
            for elem in data:
                ...
                if (
                    "client" in data[i]["data"]["label"]
                    or "server" in data[i]["data"]["label"]
                ):
                    data[i]["data"]["kind"] = "TelcoCloudVirtualDevice"
                elif (
                    "frame" in data[i]["data"]["label"]
                    or "packet" in data[i]["data"]["label"]
                ):
    ...
    def serve_attack():
        ...
        def call_python_version(Version, Module, Function, ArgumentList):
            folder = (
                PANTHER.SOURCE_DIR
                + "/panther-ivy/protocol-testing/quic/quic_tests/server_tests/"
            )
            IvyServer.experiments.update_includes_ptls()
            IvyServer.experiments.update_includes()
            # //env:PATH=%s//env:PYTHONPATH=%s , os.getenv('PATH'), os.getenv('PYTHONPATH'))
            gw = execnet.makegateway(
                "popen//python=python%s//chdir=%s" % (Version, folder)
            )
            # os.chdir(folder)
        ...
        def buildEdges(relationRecord):
            data = {
                "source": str(relationRecord.r.start_node._id),
