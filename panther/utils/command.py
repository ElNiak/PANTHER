...
class ShellCommand:
    ...
    def __repr__(self):
        return f"ShellCommand(command={self.command!r}, description={self.description!r}, " \
    ...
    def make_safe(self):
        ...
        if self.is_control_structure and not self.is_multiline:
            ...
            if (cmd_lower.startswith('while ') or 
                cmd_lower.startswith('for ') or 
                cmd_lower.startswith('if ') or 
                cmd_lower.startswith('case ') or
                'function ' in cmd_lower or '() {' in cmd_lower):
                
                # Check if the control structure is likely incomplete 
                # (missing done/fi/esac or containing multiline indicators)
                if ((cmd_lower.startswith('while ') and ('do' not in cmd_lower or 'done' not in cmd_lower)) or
                    (cmd_lower.startswith('for ') and ('do' not in cmd_lower or 'done' not in cmd_lower)) or
                    (cmd_lower.startswith('if ') and ('then' not in cmd_lower or 'fi' not in cmd_lower)) or
                    (cmd_lower.startswith('case ') and ('in' not in cmd_lower or 'esac' not in cmd_lower)) or
                    (('function ' in cmd_lower or '() {' in cmd_lower) and '}' not in cmd_lower) or
                    ('\n' in self.command) or (';' in self.command)):
    ...
    def from_string(cls, command_str: str, is_critical: bool = True) -> "ShellCommand":
        ...
        if cmd_parts:
            ...
            elif any(command_str.strip().startswith(f"{keyword} ") for keyword in ['while', 'for', 'if', 'case', 'function']):
                is_control_structure = True
                is_multiline = True
        ...
        if not is_control_structure and not is_function_def:
            ...
            if ((' while ' in cmd_lower or cmd_lower.startswith('while ')) or
                (' for ' in cmd_lower or cmd_lower.startswith('for ')) or
                (' if ' in cmd_lower or cmd_lower.startswith('if ')) or
                (' case ' in cmd_lower or cmd_lower.startswith('case '))):
                is_control_structure = True
                is_multiline = True
            # Also check for typical patterns of control structures in the first line
            elif any(command_str.strip().startswith(f"{keyword} ") for keyword in ["while", "for", "if", "case"]):
                is_control_structure = True
                is_multiline = True
        ...
        if ('"' in command_str and "'" in command_str) or \
           (command_str.count('"') > 2) or \
           (command_str.count("'") > 2):
            has_nested_quotes = True
            
        # Special case for export commands with nested quotes
        if cmd_parts and cmd_parts[0] == 'export' and '=' in command_str and \
           ('"' in command_str or "'" in command_str):
            has_nested_quotes = True
...
def escape_shell_command(cmd: str) -> str:
    ...
    # Special handling for commands with redirection operators (>, >>, <, <<)
    for op in ['>', '>>', '<', '<<']:
        if op in cmd:
            # More advanced redirection handling:
            # For simple redirections, keep the command as-is
            # For complex commands with redirections, we need to ensure the command part is properly escaped
            # while preserving the redirection structure
            
            # Split the command based on the redirection operator
            parts = cmd.split(op, 1)
            if len(parts) == 2:
                cmd_part = parts[0].strip()
                redir_part = parts[1].strip()
                
                # Check if the command part needs quoting
                cmd_parts = cmd_part.split()
                if not cmd_parts:
                    # Empty command with redirection, shouldn't happen, but return as is
                    return cmd
                
                # If it starts with a shell builtin or control structure, leave it as is
                if cmd_parts[0] in SHELL_BUILTINS or cmd_parts[0] in SHELL_CONTROL_STRUCTURES:
                    return cmd
                
                # If it contains other special characters, quote just the command part
                if any(special_op in cmd_part for special_op in ['&&', '||', ';', '&', '|']):
                    # For complex commands, we might need more sophisticated handling
                    # For now, return as-is to avoid breaking anything
                    return cmd
                
            # Default case - don't quote commands with redirections
            return cmd
        ...
        for part in cmd.split(";"):
            ...
            if stripped:
                ...
                if part_cmd in SHELL_BUILTINS or part_cmd in SHELL_CONTROL_STRUCTURES:
                    ...
                    # Normalize each part individually as well
                    # normalized = normalize_command_ending(stripped)
    ...
    # but not at the end (those were handled by normalize_command_ending)
    for op in ["&&", "||"]:
        if op in cmd:
            parts = []
...
def format_multiline_command(cmd: str) -> str:
    ...
    Returns:
        Command wrapped in a heredoc for safe execution
    """
    ...
    # Handle any trailing control operators
    # cmd = normalize_command_ending(cmd)
    
    # Check if this is a control structure that might be missing its closing keyword
    # Look for 'while'/'for'/'if'/'case' without matching 'done'/'fi'/'esac'
    if cmd.strip().startswith(('while ', 'for ', 'if ', 'case ')):
        lines = cmd.strip().split('\n')
        first_line = lines[0].strip()
        
        # More detailed pattern matching for control structures
        if first_line.startswith('while '):
            # Check for missing 'do' keyword
            if 'do' not in cmd.lower():
                # Determine a sensible default body based on the while condition
                body = "    sleep 1"
                
                # Look for common patterns in while loops
                if 'exists' in first_line or '-e ' in first_line or '-f ' in first_line:
                    # File existence checking loop
                    body = "    echo 'Waiting for file...'\n    sleep 1"
                elif 'test ' in first_line or '[ ' in first_line or '[[ ' in first_line:
                    # Generic test condition
                    body = "    echo 'Condition not yet met'\n    sleep 1"
                
                cmd += f"\ndo\n{body}\ndone"
                
            # Check for missing 'done' keyword
            elif 'do' in cmd.lower() and 'done' not in cmd.lower():
                # Special handling for commands that end with 'do'
                if cmd.strip().endswith('do') or cmd.strip().endswith('do;'):
                    # Determine a sensible default body based on the while condition
                    body = '    sleep 1'
                    ...
                    cmd += f'\n{body}\ndone'
                # If the loop already has some content after 'do', just add the 'done'
                elif '\ndo' in cmd or 'do\n' in cmd:
                    cmd += "\ndone"
                else:
                    # Otherwise add a sensible default body
                    cmd += "\n    sleep 1\ndone"
                
        elif first_line.startswith('for '):
            # Check for missing 'do' keyword
            if 'do' not in cmd:
                if 'in ' in first_line:  # Regular for loop
                    cmd += "\ndo\n    echo \"Processing item\"\ndone"
                else:  # C-style for loop
                    cmd += "\ndo\n    echo \"Loop iteration\"\ndone"
            # Check for missing 'done' keyword
            elif 'do' in cmd and 'done' not in cmd:
                cmd += "\n    echo \"Processing item\"\ndone"
                
        elif first_line.startswith('if '):
            # Check for missing 'then' keyword
            if 'then' not in cmd:
                cmd += "\nthen\n    echo 'Condition met'\nfi"
            # Check for missing 'fi' keyword
            elif 'then' in cmd and 'fi' not in cmd:
                # Check for else clause
                if 'else' in cmd:
                    cmd += "\n    echo 'Alternative action'\nfi"
                else:
                    cmd += "\n    echo 'Default action'\nfi"
                    
        elif first_line.startswith('case '):
            # Check for missing 'in' keyword
            if 'in' not in cmd:
                cmd += "\nin\n    *) echo 'Default action';;\nesac"
            # Check for missing 'esac' keyword
            elif 'in' in cmd and 'esac' not in cmd:
                cmd += "\n    *) echo 'Default action';;\nesac"
                
        # Handle function definitions that might be incomplete
        elif ('function ' in first_line or '() {' in first_line) and '}' not in cmd:
            cmd += "\n    echo 'Function body'\n}"
...
def format_function_definition(cmd: str) -> str:
    ...
    Returns:
        Formatted function definition with proper syntax
    """
    # Ensure the function definition doesn't have leading/trailing whitespace
    cmd = cmd.strip()
    
    # Normalize the command ending to remove any control operators
    cmd = normalize_command_ending(cmd)
...
def convert_legacy_commands(commands: List[str]) -> List[ShellCommand]:
    ...
    This function helps migrate from the old string-based command format
    to the new structured ShellCommand format. Empty commands are filtered out.

    Args:
        commands: List of command strings from the old format

    Returns:
        List of ShellCommand objects with empty commands filtered out
    """
    result = []
    ...
    for cmd in commands:
        ...
        if isinstance(cmd, str) and cmd.strip().startswith("#") and not cmd.strip().startswith("# PANTHER_"):
            ...
            if actual_cmd.strip():
                result.append(
                    ShellCommand(
                        ...
                        is_critical=False,
                        is_multiline="\n" in actual_cmd or actual_cmd.count(";") > 1 or actual_cmd.count("&&") > 1,
                    )
                )
        # Check if this is a function definition command
            ...
            # Regular command - create a ShellCommand but check if it's empty
            shell_cmd = ShellCommand.from_string(cmd)
            if not shell_cmd.is_empty:
...
def add_template_filters(env):
    ...
    This function adds template filters that can be used in Jinja2 templates
    across all PANTHER plugins for consistent command processing.
