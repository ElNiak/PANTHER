...
def apply_scalability(service_name, threshold, scale_factor, stack_name="panther"):
    ...
    service_list = (
        subprocess.check_output(["docker", "stack", "services", stack_name])
        .decode()
        .split()
    )
    ...
    while threshold_not_reached(avg_cpu, threshold):
        ...
        logging.info(
            f"[{service_full_name}] {avg_cpu} < {threshold} ? Yes, threshold not reached."
        )
        time.sleep(2)

    logging.info(
        f"[{service_full_name}] {avg_cpu} < {threshold} ? No, threshold reached!"
    )
    new_replicas = replicas * scale_factor
    subprocess.check_call(
        ["docker", "service", "scale", f"{service_full_name}={new_replicas}"]
    )
