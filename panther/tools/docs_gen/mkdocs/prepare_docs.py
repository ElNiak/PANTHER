...
def main():
    ...
    for readme in repo_dir.glob("panther/**/README.md"):
        ...
        if not any(
            subdir in str(readme)
            for subdir in [
                "/panther_ivy/submodules/",
                "/panther_ivy/test/",
                "/panther_ivy/ivy/",
                "/panther_ivy/protocol-testing/",
            ]
        ):
    ...
    for md_file in repo_dir.glob("panther/**/*.md"):
        if md_file.name.lower() != "readme.md" and not any(
            subdir in str(md_file)
            for subdir in [
                "/panther_ivy/submodules/",
                "/panther_ivy/test/",
                "/panther_ivy/ivy/",
    ...
    if not index_path.exists():
        with open(index_path, "w") as f:
            ...
            f.write(
                "Welcome to the PANTHER (Protocol formal Analysis and formal Network Threat Evaluation Resources) documentation.\n\n"
            )
            f.write("## Quick Links\n\n")
