...
class ExternalDependencyResolver:
    ...
    def _parse_spec(self, dep_spec: str) -> tuple[str, str | None]:
    ...
    def _extract_version(self, output: str, patterns: list[str]) -> str | None:
    ...
    def _check_version_spec(self, current_version: str, version_spec: str | None) -> bool:
    ...
    def _check_docker(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Docker version {version_str} does not satisfy {version_spec}"
    ...
    def _check_docker_compose(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        for cmd in [["docker-compose", "--version"], ["docker", "compose", "version"]]:
            ...
            if success:
                version_str = self._extract_version(
                    output,
                    [r"docker-compose version ([0-9.]+)", r"Docker Compose version v?([0-9.]+)"],
                )
                if version_str and version_spec:
                    if not self._check_version_spec(version_str, version_spec):
                        return (
    ...
    def _check_cmake(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"CMake version {version_str} does not satisfy {version_spec}"

        return True, f"CMake {version_str} is available"

    def _check_z3(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Z3 version {version_str} does not satisfy {version_spec}"

        return True, f"Z3 {version_str} is available"

    def _check_strace(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"strace version {version_str} does not satisfy {version_spec}"

        return True, f"strace {version_str} is available"

    def _check_gperf(self, version_spec: str | None) -> tuple[bool, str]:
        """Check Google Performance Tools installation."""
        # Check for gperftools profiler
        success, _ = self._run_command(["pprof", "--help"])
        if not success:
            return False, "Google Performance Tools (gperftools) is not installed or not in PATH"
    ...
    def _check_valgrind(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Valgrind version {version_str} does not satisfy {version_spec}"

        return True, f"Valgrind {version_str} is available"

    def _check_python(self, version_spec: str | None) -> tuple[bool, str]:
    ...
    def _check_rust(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Rust version {version_str} does not satisfy {version_spec}"

        return True, f"Rust {version_str} is available"

    def _check_cargo(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Cargo version {version_str} does not satisfy {version_spec}"

        return True, f"Cargo {version_str} is available"

    def _check_git(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Git version {version_str} does not satisfy {version_spec}"

        return True, f"Git {version_str} is available"

    def _check_gcc(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"GCC version {version_str} does not satisfy {version_spec}"

        return True, f"GCC {version_str} is available"

    def _check_gpp(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"G++ version {version_str} does not satisfy {version_spec}"

        return True, f"G++ {version_str} is available"

    def _check_clang(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Clang version {version_str} does not satisfy {version_spec}"

        return True, f"Clang {version_str} is available"

    def _check_make(self, version_spec: str | None) -> tuple[bool, str]:
        ...
        if version_str and version_spec:
            if not self._check_version_spec(version_str, version_spec):
                return False, f"Make version {version_str} does not satisfy {version_spec}"

        return True, f"Make {version_str} is available"

    def _check_generic_command(self, command: str, version_spec: str | None) -> tuple[bool, str]:
