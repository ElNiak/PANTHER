...
class WorkflowStateTracker(LoggerMixin):
        ...
        with self._lock:
            ...
            if current_state is None:
                if state == WorkflowState.CREATED:
                    ...
                    self._record_transition(experiment_id, None, state, "experiment_created")
                    self.logger.info(
            ...
            if self._validate_transition(current_state, state):
                ...
                self._workflow_states[experiment_id] = state
                self._record_transition(experiment_id, old_state, state, "workflow_transition")
                self.logger.info(
    ...
    def get_workflow_state(self, experiment_id: str) -> WorkflowState | None:
        """
    ...
    def force_fail_workflow(self, experiment_id: str, reason: str = "Forced failure") -> bool:
        """
        ...
        with self._lock:
            current_state = self._workflow_states.get(experiment_id)
            if current_state is None:
                self.logger.error(f"Cannot force fail non-existent workflow '{experiment_id}'")
                return False
    ...
    def clear_workflow_state(self, experiment_id: str) -> None:
        ...
        with self._lock:
            if experiment_id in self._workflow_states:
                ...
                self._record_transition(experiment_id, old_state, None, "workflow_cleared")
    ...
    def get_all_workflow_states(self) -> dict[str, str]:
        ...
        """
        with self._lock:
            return {exp_id: state.value for exp_id, state in self._workflow_states.items()}
    ...
    def _validate_transition(self, current_state: WorkflowState, new_state: WorkflowState) -> bool:
