...
def escape_shell_command_without_redirections(cmd: str) -> str:
    ...
    try:
        ...
        # Quote the entire command if it contains special characters
...
def combine_shell_constructs(command_list: List[str]) -> str:
    """
    Combine a list of shell commands with proper operators.

    Args:
        command_list: List of commands to combine
    ...
    commands = [cmd.strip() for cmd in command_list if cmd.strip()]
...
def combine_shell_constructs(command_list):
            ...
            if (
                ...
                if has_shell_command:
                    ...
                    if shell_cmd_template and hasattr(shell_cmd_template, "command"):
                        # Import ShellCommand here to avoid circular imports
                        from panther.core.command_processor.shell_command import (
                            ShellCommand,
                        )
    ...
    if construct_buffer:
        ...
        if has_shell_command:
            ...
            if shell_cmd_template and hasattr(shell_cmd_template, "command"):
                # Import ShellCommand here to avoid circular imports
                from panther.core.command_processor.shell_command import ShellCommand
