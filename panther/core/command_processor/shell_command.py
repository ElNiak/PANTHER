...
class ShellCommand:
    ...
    def _detect_command_type(self) -> None:
        ...
        if "#" in command_str:
            ...
            self.metadata.comment_text = comment_parts[1].strip() if len(comment_parts) > 1 else ""
        ...
        if (not self.metadata.is_function_definition and 
            not self.metadata.has_control_operators and
            len(cmd_parts) == 1 and
            "(" not in command_str):
    ...
    def get_shell_safe_command(self, escape_variables: bool = True) -> str:
        ...
        if (self.metadata.is_variable_assignment or 
            self.metadata.is_shell_builtin or
            (self.metadata.is_shell_builtin and self.executable == "export")):
    ...
    @classmethod
    def from_string(cls, command_str: str, is_critical: bool = True, **kwargs) -> "ShellCommand":
        """
        ...
        return cls(command_str, **kwargs)
    
    # Alias for backward compatibility
    @classmethod
    def create_from_string(cls, command_str: str, is_critical: bool = True) -> "ShellCommand":
