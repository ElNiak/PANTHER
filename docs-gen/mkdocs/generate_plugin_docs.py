...
""")
        ...
        if not os.path.exists(output_category_dir / "index.md"):
            ...
            with open(output_category_dir / "index.md", 'w', encoding='utf-8') as f:
                f.write(f"""# {category.capitalize()} Plugins
...
""")
                ...
                plugin_dirs = [d for d in os.listdir(category_dir) 
                              if os.path.isdir(os.path.join(category_dir, d)) and not d.startswith('__')]
        ...
        for plugin_dir in glob.glob(str(category_dir / "*")):
            ...
            if os.path.exists(readme_path):
                ...
                result = copy_readme_with_relative_links(readme_path, output_path, ROOT_DIR)
                if not result:
                    warnings_found = True
...
""")
                print(f"Warning: No README found for {category}/{subcategory}/{plugin_name}")
                warnings_found = True
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not success and args.strict:
            print("❌ Strict mode enabled: Warnings were detected during plugin documentation generation.")
            sys.exit(1)
