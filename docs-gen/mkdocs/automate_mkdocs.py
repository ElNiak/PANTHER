...
def include_markdown_files(
    ...
    # Add markdown files from docs-gen directory
    docs_gen_files = list(repo_dir.glob("docs-gen/**/*.md"))
    ...
    try:
        with open(f"{repo_dir}/{mkgendocs_f}", "r") as f:
            content = f.read()
    ...
    for entry in new_entries:
        insert_string += f'  - page: "{entry["page"]}"\n    source: "{entry["source"]}"\n\n'
    ...
    try:
        ...
        for index, line in enumerate(contents):
            if match_string.strip() in line.strip():
                ...
                while insertion_point < len(contents) and contents[insertion_point].strip() and contents[insertion_point].strip().startswith("  -"):
                    insertion_point += 1
                break
