...
def get_relative_path_to_root(file_path: Path, project_root: Path) -> str:
    ...
    try:
        ...
        levels = len(rel_path.parents) - 1  # -1 because parents includes the file itself
...
def fix_markdown_link(link_url: str, path_to_root: str, current_file: Path, project_root: Path) -> str:
    """
...
def process_markdown_file(file_path: Path, project_root: Path, dry_run: bool = True) -> List[Tuple[str, str]]:
    """
