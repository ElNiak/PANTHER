...
class LinkValidator:
    ...
    def __init__(self, root_dir: Path, docs_dir: str = DOCS_DIR, autofix: bool = False, 
                 check_external: bool = False, ignore_readme_refs: bool = False):
        ...
        self.external_cache: Dict[str, LinkStatus] = {}  # Cache external URL validation results
    ...
    def normalize_path(self, base_path: Path, link_path: str) -> Tuple[Path, Optional[str]]:
    ...
    def check_internal_link(self, link_info: LinkInfo) -> LinkStatus:
        ...
        try:
            if link_info.target_file and link_info.target_file.exists():
                ...
                if link_info.anchor:
                    with open(link_info.target_file, 'r', encoding='utf-8') as file:
                        content = file.read()
                ...
                if not link_info.target_file:
                    link_info.message = f"Invalid target file: None"
                else:
    ...
    def check_external_link(self, link_info: LinkInfo) -> LinkStatus:
        ...
        if not REQUESTS_AVAILABLE:
            link_info.message = "Package 'requests' not available, skipping external link check"
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for match in re.finditer(LINK_PATTERN, content):
                    ...
                    if link_url.startswith(('http://', 'https://', 'ftp://')):
                        ...
                        if link_url.startswith(GITHUB_REPO_URL) and '/blob/' in link_url:
                            try:
                    ...
                    elif file_path.name == "README.md" and re.match(r'^10\.\d+/', link_url):
    ...
    def validate_links(self) -> None:
        ...
        print(f"Found {self.total_links} links to validate ({len(ignored_links)} ignored)")
        
        # Validate all internal links
        for link_info in all_links:
            if link_info.link_type in [LinkType.INTERNAL, LinkType.IMAGE, LinkType.ANCHOR]:
        ...
        if self.check_external:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_EXTERNAL_CHECK_THREADS) as executor:
                future_to_link = {executor.submit(self.check_external_link, link): link for link in external_links}
                for future in concurrent.futures.as_completed(future_to_link):
                    link = future_to_link[future]
                    try:
                        link.status = future.result()
                        if link.status != LinkStatus.OK and link.status != LinkStatus.UNVALIDATED:
    ...
    def fix_issues(self) -> None:
        ...
        files_to_update = {}  # Map of file path -> list of (old_text, new_text) replacements
        
        for link_info in self.link_issues:
            fixed = False
            
            # Try to fix GitHub URLs to relative links
            if link_info.link_type == LinkType.INTERNAL and link_info.raw_link.startswith(GITHUB_REPO_URL):
                try:
            ...
            if not fixed and link_info.link_type == LinkType.INTERNAL and link_info.status == LinkStatus.BROKEN:
    ...
    def print_report(self) -> None:
        ...
        for link in self.link_issues:
            if link.link_type == LinkType.INTERNAL and link.status in [LinkStatus.BROKEN, LinkStatus.UNFIXABLE]:
            ...
            elif link.link_type == LinkType.EXTERNAL and link.status == LinkStatus.BROKEN:
        ...
        if broken_internal:
            ...
            for link in broken_internal:
                ...
                print(f"  • {rel_source}:{link.line_number} - [{link.link_text}]({link.raw_link})")
...
def main():
    ...
    parser.add_argument('--autofix', action='store_true', help='Automatically fix detected issues when possible')
    parser.add_argument('--check-external', action='store_true', help='Check external URLs (might be slow)')
    parser.add_argument('--ignore-readme-refs', action='store_true', 
