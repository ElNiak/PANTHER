...
def parse_arguments():
    ...
    parser.add_argument('--mkdocs-yaml', type=str,
                        help='Path to the mkdocs.yml file to fix')
...
def fix_yaml_config(config_path):
    ...
    content = re.sub(r'!!python/name:pymdownx\.emoji\.([^\s]+)', r'"pymdownx_emoji_\1"', content)
    content = re.sub(r'!!python/name:pymdownx\.superfences\.([^\s]+)', r'"pymdownx_superfences_\1"', content)
    content = re.sub(r'!!python/name:pymdownx\.([^\s]+)', r'"pymdownx_\1"', content)
    content = re.sub(r'!!python/object/apply:pymdownx\.([^\s]+)', r'"pymdownx_object_\1"', content)
...
def main():
    ...
    if args.mkdocs_yaml:
        ...
        # Default: assume we're in the docs-gen directory
