#!/usr/bin/env python3
# filepath: /Users/elniak/Documents/Project/PANTHER/docs-gen/verify_docs.py
...
def verify_docs(root_dir, report_mode=False, ignore_dirs=None):
    ...
    for path in Path(root_dir).rglob('*.md'):
        ...
        if todo_verify_matches:
            is_valid = False
            issues.append(f"{path}: Contains {len(todo_verify_matches)} unresolved verification markers (TODO:VERIFY)")
...
def main():
    parser = argparse.ArgumentParser(description='Verify documentation source references and markers.')
    parser.add_argument('--root', default='docs', help='Root directory to search for markdown files')
    parser.add_argument('--report', action='store_true', help='Report mode: only report issues without failing')
    parser.add_argument('--ignore', nargs='*', default=['node_modules', '.git', 'venv'], 
