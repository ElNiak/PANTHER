...
def cross_reference_root_docs(markdown_files: Dict[str, List[Path]], dry_run: bool = False) -> int:
    ...
    for plugin_type, title in PLUGIN_TYPES.items():
        for file_path in markdown_files[plugin_type]:
            rel_path = str(file_path.relative_to(REPO_ROOT))
            if rel_path.endswith("README.md") and f"plugins/{plugin_type}/README.md" == rel_path:
    ...
    for file_name, title in CORE_DOCS.items():
        ...
        if dry_run:
            print(f"Would add {len(refs)} references to {file_path.relative_to(REPO_ROOT)}")
        else:
    ...
    for plugin_type, readme_path in plugin_readmes.items():
        ...
        if dry_run:
            print(f"Would add {len(refs)} references to {readme_path.relative_to(REPO_ROOT)}")
        else:
...
def cross_reference_plugin_docs(markdown_files: Dict[str, List[Path]], dry_run: bool = False) -> int:
    ...
    for plugin_type in PLUGIN_TYPES.keys():
        ...
        refs = [(f"{subcat.title()} {plugin_type[:-1].title()}", path) 
                for subcat, path in subcategory_readmes.items()]
                
        if dry_run:
            print(f"Would add {len(refs)} references to {dev_guide.relative_to(REPO_ROOT)}")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='PANTHER Documentation Cross-Referencer')
