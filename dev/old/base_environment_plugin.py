...
from panther.plugins.plugin_interface_enhanced import IPlugin
...
class BaseEnvironmentPlugin(
    IPlugin, EnvironmentPluginEventMixin, EnhancedEnvironmentPluginEventMixin
):
    """
    ...
    def __init__(self, plugin_id: str = None, name: str = None, config: dict[str, Any] = None):
        """
        ...
        self.logger = logging.getLogger(f"EnvironmentPlugin.{self.plugin_id}")
        self._status = {
            "state": "created",
            "details": {},
        }
    ...
    def shutdown(self) -> bool:
        """
        Shutdown the environment plugin and clean up resources.
        ...
        try:
            ...
            for resource_id in resource_ids:
                if self.resources[resource_id].get("status") == "allocated":
                    self.release_resource(resource_id)

            # Teardown the environment if it's been set up
            if self._status["state"] in ["running", "setup"]:
                result = self.stop()
                if not result:
                    self.logger.warning(
                        f"Environment plugin {self.plugin_id} failed to stop during shutdown"
                    )
            ...
            self.logger.exception(
                f"Error during shutdown of environment plugin {self.plugin_id}: {e}"
            )
            self._status["state"] = "shutdown_failed"
            self._status["details"]["error"] = str(e)
            return False
    ...
    def _shutdown_environment(self) -> None:
        """
        Perform environment-specific shutdown operations.

        This method is called during plugin shutdown to allow
        environment implementations to clean up resources.
