...
This script converts code like:
    logger.info(f"Some messdef has_logging_fstrings_regex(file_path: str) -> bool:
    try:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs='*',
        default=[],
        help='List of specific files to process (default: all Python files in directory)'
    )
    parser.add_argument(
        '--exclude',
...
class FStringVisitor(ast.NodeVisitor):
    ...
    def is_logger_object(self, node):
        ...
        if isinstance(node, ast.Name) and ('logger' in node.id or node.id.endswith('_logger') or "lo"):
...
def find_fstring_logging_calls(content: str) -> List[Tuple[int, int, str, int]]:
    ...
    while line_number < len(lines):
        ...
        # Matches patterns like: logger.info(f"..."), self.logger.info(f"..."), _logger.info(f"..."), logging.debug(f"...")
        ...
        if match:
            ...
            if found_fstring:
                # Additional verification with a simpler pattern to ensure the f-string is actually part of the logging call
                # We remove the [^)]* part which might be too restrictive
                ...
                if re.search(fstring_pattern, current, re.DOTALL):
                    # Check if the f-string is in the arguments (could be first or another position)
...
def fix_fstring_logging(match: str, indentation: int) -> str:
    ...
    try:
        ...
        fstring_on_first_line = 'f"' in match[:first_line_end] or "f'" in match[:first_line_end]
...
def process_file(file_path: str, dry_run: bool = False, verbose: bool = False) -> bool:
    ...
    try:
        ...
        if ("logging." not in content and 'logger.' not in content and '_logger.' not in content) or ('f"' not in content and "f'" not in content):
            print(f"No f-string logging found in {file_path}. Skipping.")
            return False
    ...
    if args.files:
        ...
        for file_path in args.files:
            ...
            try:
                ...
                print(f"Checking file: {file_path} - {'has issues' if has_issues else 'no issues'}")
            except Exception as e:
        ...
        # If no specific files are provided, process all Python files in the directory
