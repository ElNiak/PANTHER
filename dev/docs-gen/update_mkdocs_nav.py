...
def setup_yaml_handlers():
    ...
    yaml.add_constructor(
        "!!python/name", python_name_constructor, Loader=yaml.SafeLoader
    )
    yaml.add_constructor(
        "tag:yaml.org,2002:python/object/apply",
        python_object_apply_constructor,
...
def load_mkdocs_config() -> dict:
    ...
    try:
        ...
        with open(MKDOCS_CONFIG, encoding="utf-8") as file:
    ...
    print(
        "could not determine a constructor for the tag 'tag:yaml.org,2002:python/name'"
    )
    print(
        "This error often occurs with pymdownx extensions. Check your mkdocs.yml file."
    )
    return {}
    ...
    for category, title in PLUGIN_CATEGORIES.items():
        ...
        if category_files:
            ...
            for file in category_files:
                ...
                if (
                    parts[-1] == "README.md" and len(parts) == 3
                ):  # plugins/category/README.md
                    general_files.append({"Overview": rel_path})
                # Handle development.md for the plugin category
                elif (
                    parts[-1] == "development.md" and len(parts) == 3
                ):  # plugins/category/development.md
                    general_files.append({"Development": rel_path})
                # Handle subcategory files
                    ...
                    if parts[-1] == "README.md" or parts[-1] == "index.md":
                        subcategories[subcategory].insert(0, {"Overview": rel_path})
                    else:
                        file_title = get_title_from_markdown(file)
                        subcategories[subcategory].append({file_title: rel_path})
...
def build_updated_nav(markdown_files: list[Path], current_config: dict) -> list:
        ...
        for section, patterns in DEFAULT_SECTIONS.items():
            for pattern in patterns:
                if (
                    ...
                    or file.name.lower() == pattern.lower()
                ):
    ...
    for title, entries in plugin_nav.items():
        if entries:
            ...
            plugin_index = next(
                (i for i, item in enumerate(nav) if "Plugin System" in item), -1
            )
            if plugin_index >= 0:
