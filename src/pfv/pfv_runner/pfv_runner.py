...
class Runner:
    def __init__(self, config, protocol_config, current_protocol, implems, executed_test=[]):
        # Setup logger
        self.log = logging.getLogger("pfv-runner")
        ...
        # TODO refactor
        self.iters = self.config["global_parameters"].getint("iter")  # Number of iteration per test           # TODO enforce in this file
    ...
    def save_shadow_binaries(self,implem, test,run_id):
        ...
        if self.config["net_parameters"].getboolean("shadow"):
            ...
            if test.mode == "client":
                ...
                index = 1
            binary_path = self.implems[implem][index][implem]["binary-dir"].replace("$IMPLEM_DIR",IMPLEM_DIR.replace("$PROT",self.current_protocol)).replace("$MODEL_DIR",MODEL_DIR) + "/" + \
                          self.implems[implem][index][implem]["binary-name"].replace("$IMPLEM_DIR",IMPLEM_DIR.replace("$PROT",self.current_protocol)).replace("$MODEL_DIR",MODEL_DIR).split(" ")[-1]
            binary_name = self.implems[implem][index][implem]["binary-name"].replace("$IMPLEM_DIR",IMPLEM_DIR.replace("$PROT",self.current_protocol)).replace("$MODEL_DIR",MODEL_DIR).split(" ")[-1].split("/")[-1]
            os.system("cp " + binary_path + " " + self.config["global_parameters"]["dir"]+str(run_id) + "/" + binary_name)
            os.system("cp " + self.config["global_parameters"]["build_dir"]  + test.name + " " + self.config["global_parameters"]["dir"]+str(run_id) + "/" + test.name)
            
            
    # Return dictionnary of paths according to possible location
    # TODO make more robust
    def get_implementation_dir(self,implem):
        return self.implems[implem][0][implem]["binary-dir"].replace("$IMPLEM_DIR",IMPLEM_DIR.replace("$PROT",self.current_protocol)), \
               self.implems[implem][1][implem]["binary-dir"].replace("$IMPLEM_DIR",IMPLEM_DIR.replace("$PROT",self.current_protocol))
    ...
    def config_pcap(self, ivy_dir, implem, test):
        if self.config["net_parameters"].getboolean("vnet"):
            pcap_name = ivy_dir +"/ivy_lo_"+ implem +"_"+ test.replace(".ivy","") +".pcap"
    ...
    def save_shadow_res(self, test, i, pcap_name,run_id):
        if self.config["net_parameters"].getboolean("shadow"):
            ...
            if "client" in test.mode: # TODO
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/server/*.stderr >>"+ os.path.join(self.config["global_parameters"]["dir"]+str(run_id), 'ivy_stderr.txt'))
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/server/*.stdout >>"+ os.path.join(self.config["global_parameters"]["dir"]+str(run_id), test.name+str(i)+'.iev'))   
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/client/*.stdout >>"+ os.path.join(self.config["global_parameters"]["dir"]+str(run_id), test.name+str(i)+'.out'))
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/client/*.stderr >>"+ os.path.join(self.config["global_parameters"]["dir"]+str(run_id), test.name+str(i)+'.err'))
            elif "server" in test.mode:
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/client/*.stderr >>" + os.path.join(self.config["global_parameters"]["dir"]+str(run_id), 'ivy_stderr.txt'))
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/client/*.stdout >> "+ os.path.join(self.config["global_parameters"]["dir"]+str(run_id), test.name+str(i)+'.iev'))
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/server/*.stdout >>"+ os.path.join(self.config["global_parameters"]["dir"]+str(run_id), test.name+str(i)+'.out'))
                os.system("cat "+self.config["global_parameters"]["dir"]+str(run_id) +"/shadow.data/hosts/server/*.stderr >>"+ os.path.join(self.config["global_parameters"]["dir"]+str(run_id), test.name+str(i)+'.err'))
