...
class CoAPIvyTest(IvyTest):
    ...
    def generate_tester_command(self, iteration, iclient):
        ...
        return ' '.join(['{}{}{}/{} seed={} server_port={} server_addr={}  {}'.format(timeout_cmd,prefix,self.config["global_parameters"]["build_dir"],self.name,randomSeed,server_port,ip_server,'' 
            if self.is_client else 'client_port={}  client_addr={}'.format(client_port,ip_client))] + self.extra_args + ([""] if self.config["net_parameters"].getboolean("vnet") else [""])) 