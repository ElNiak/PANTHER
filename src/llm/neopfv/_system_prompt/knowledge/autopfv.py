...
and` when the user
    ...
    def new_protocol_integration():
        ...
        instruction = inspect.cleandoc(
            ...
            1. Apply step A from `experts_methodology` to step 1 from `new_project` for protocol asked
            2. Apply step B from `experts_methodology` to step 1 from `new_project` for protocol asked
            3. Apply step C from `experts_methodology` to step 1 from `new_project` for protocol asked
            4. Apply step D from `experts_methodology` to step 1 from `new_project` for protocol asked
            5. Apply step E from `experts_methodology` to step 1 from `new_project` for protocol asked
            
            6. Apply step A from `experts_methodology` to step 2 from `new_project` for protocol asked
            7. Apply step B from `experts_methodology` to step 2 from `new_project` for protocol asked
            8. Apply step C from `experts_methodology` to step 2 from `new_project` for protocol asked
            9. Apply step D from `experts_methodology` to step 2 from `new_project` for protocol asked
            10. Apply step E from `experts_methodology` to step 2 from `new_project` for protocol asked
            
            11. Apply step A from `experts_methodology` to step 3 from `new_project` for protocol asked
            12. Apply step B from `experts_methodology` to step 3 from `new_project` for protocol asked
            13. Apply step C from `experts_methodology` to step 3 from `new_project` for protocol asked
            14. Apply step D from `experts_methodology` to step 3 from `new_project` for protocol asked
            15. Apply step E from `experts_methodology` to step 3 from `new_project` for protocol asked
            
            16. Apply step A from `experts_methodology` to step 4 from `new_project` for protocol asked
            17. Apply step B from `experts_methodology` to step 4 from `new_project` for protocol asked
            18. Apply step C from `experts_methodology` to step 4 from `new_project` for protocol asked
            19. Apply step D from `experts_methodology` to step 4 from `new_project` for protocol asked
            20. Apply step E from `experts_methodology` to step 4 from `new_project` for protocol asked
            """
        )
        return instruction
...
def _slash_command(command: str) -> None:
    ...
    command_func = getattr(NeoPFV, command, None)
...
def _setup():
    ...
    instructions = inspect.cleandoc(
        ...
        """
        ## ChatGPT NeoPFV , version {AUTODEV_VERSION}
        ...
        ## Response
        Adopt NeoPFV Role: You are NeoPFV, the team leader that orchestrates a dynamic, interactive dialogue between three different logical experts who collaboratively produce the new formal model for <new_prot>: an Ivy programmer, a critic, and a network & formal specification expert. 
        ...
        **Take note**:
        ...
        6. Now turn control over to the user, and stay in character as NeoPFV from now on.
        '''
    )
    instructions_rerun = inspect.cleandoc(
        """
        Inform the user that the NeoPFV  environment has been reloaded, and return control over to the user.
        """
    )
...
if __name__ == "__main__":
    ...
    autodev_functions = _get_methods_and_docstrings(NeoPFV)
    _setup()
