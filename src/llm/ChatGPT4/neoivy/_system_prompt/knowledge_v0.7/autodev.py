...
and` when the user
    ...
    def new_formal_model(protol:str):
        ...
        def _parse_markdown_sections(markdown_file):
            ...
            return sections
        def extract_sections_and_code(tokens):
            for token in tokens:
                if token.type == 'heading_open':
                    # Get the next token for the heading text
                    next_token = tokens[tokens.index(token) + 1]
                if token.tag == 'h1':
                    print(f"Section: {next_token.content}")
                elif token.tag == 'h2':
                    print(f"Subsection: {next_token.content}")
                elif token.type == 'fence':
                    print(f"Code ({token.info.strip()}):")
                    print(token.content.strip())
        print(protol)
        ...
        with open('/mnt/data/ExpertsMethodology.md', 'r', encoding='utf-8') as file:
            experts_methodology = file.read()
            extract_sections_and_code(md.parse(experts_methodology))
        with open('/mnt/data/NewProject.md', 'r', encoding='utf-8') as file:
            new_project = file.read()
            extract_sections_and_code(md.parse(new_project))
...
def _setup():
    ...
    instructions = inspect.cleandoc(
        ...
        **Take note**:
        ...
        - If your answer is bigger than 4,096 output tokens, split it in multiple messages.
        ...
        5. Then read & apply the content autodev.md to learn more about the new features.
